...
1. ~~**購買一律要按確認。**~~ **2026-09-09 使用者改主意：確認做成選項、預設關。**
   預設跟原作一樣是一下成交；有疑慮的玩家自己去設定打開，還可以只對超過某個
   金額的才問（`confirmAbove`，0 ＝ 每筆都問）。
   ⚠ **天價保險攔下來的那幾筆一律強制確認，開關關不掉** —— 那道保險存在的理由
   就是「自動買下去會出事」，被開關關掉就沒有意義了。
   技術前提：`ConfirmCommoditiesPurchase` **沒有**硬體事件閘（只有
   `StartCommoditiesPurchase` 有），所以報價事件裡直接成交是可行的
   —— 這點是讀原作才確認的，它整支就是這樣運作。
...
## 整批瀏覽的報價**不夠拿來買**（2026-09-09 實測）

`SearchForItemKeys`（搜尋全部）回來的是均價與總量，而且**不告訴你這件東西是不是
商品**（`isCommodity`）。拿那份資料直接走購買流程，會落到非商品分支去讀
`GetItemSearchResultInfo`，那支要有**單筆查詢**（`SendSearchQuery`）的結果才有值 ——
於是明明列上寫著「單價 4799 金、在售 704」，按下購買卻回報「拍賣場上沒有人賣」。

判準要改成「有沒有跑過單筆查詢」（自己記一張 `detailed[itemID]`），不是「有沒有報價」。
沒跑過就先送單筆查詢、請玩家再按一次。

## 狀態列的字沒人看得到

視窗標題列那行狀態字小、顏色暗、在角落，玩家回報「提示訊息很不明顯，乾脆直接寫
訊息都比較明顯」。→ `SetStatus(text, alert)`：**要玩家知道才動得下去的**那幾條
（沒人賣、錢不夠、再按一次購買、買到了、失敗）除了狀態列還直接 `ns.Print` 到聊天列，
狀態列本身也改白字、alert 時轉紅。純進度（搜尋中、報價已更新）不吵。

## 一定有一個配方被選著

開視窗預設選第一個配方，下面只列它的材料；換配方＝點另一列。
不做「全部混在一起」的檢視 —— 混在一起沒辦法「先把這一樣買齊」（使用者要求）。
額外物品也是選取對象（key 是 `extra:<itemID>`）。
...
## ⚠ 拍賣場的事件是全域的（2026-09-09 實測）

玩家自己在暴雪的拍賣場介面買東西、自己按搜尋，我們一樣收得到
`COMMODITY_PURCHASE_SUCCEEDED` / `AUCTION_HOUSE_BROWSE_FAILURE`。
沒有加閘的話會噴出「已購買 ？ ×1」（我們沒有 pending，名字取不到）
跟「拍賣場沒有回應」—— 人家買成功了，我們在旁邊喊失敗。
...
還是滿滿的「可選」），使用者的結論是：**這個資訊不值得標，玩家自己判斷就好**。
清單的欄位與下單頁提示的 `(可選)` 全部移除。
留在下面的段落是當初為什麼會標錯的紀錄。

## （已移除）「可選」講的是配方，不是誰來出

`類別` 欄原本把訂單裡「顧客不一定要出」的材料標成可選，結果個人／公會訂單
（欄位大多是 Any）整張單全部標成「可選」，而畫面上每一樣明明都得備齊。
語意改成「配方需不需要這樣材料」：基礎材料一律必備，只有裝飾／加成材料才是可選。
...
`C_AuctionHouse.MakeItemKey(itemID)` 給的是 0，拿去 `SendSearchQuery` 一筆掛單
都查不到。症狀：列上明明有價格與在售數量，按購買卻說「拍賣場上沒有人賣」，
而且**批次購買會把那一樣默默跳掉**（實測：珍獸獸皮）。
→ 查詢一律優先用整批瀏覽結果帶回來的 `itemKey`。
→ 順帶：批次裡自動跳過的要計數，結束時講「其中 N 樣買不到」——
  不然「清單買完了」會讓玩家以為每一樣都到手。
