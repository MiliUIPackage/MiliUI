---
name: project_loot_history_tracking
...
v1.1 曾自行用事件記錄含難度/天賦（`AGSCDB.looted[guid][itemID][diff][spec]`），但使用者決定移除、純靠 KeystoneLoot，下賽季若 KeystoneLoot 不再提供再自行實作（做法見 git 歷史 v1.1 looted.lua）。相關 [[project_itemupgrade_preview_icon]]。
