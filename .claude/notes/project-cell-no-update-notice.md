...
metadata: 
  ...
  modified: 2026-08-17T02:47:13.652Z
---
...
⚠ **`Cell.toc` 的 `## Version: rNNN_MiliUI` 現在同時是釋出訊號**，不再只是 Revise 遷移的閘。**每次出貨 Cell 的改動都要 bump**，忘了的話提醒會安靜失效、沒有任何錯誤。分隔符是**底線**（2026-08-17 從連字號改過來），不過所有解析都是 `string.match(v, "%d+")` 只抓數字，格式改動不影響相容。
