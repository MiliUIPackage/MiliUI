...
metadata: 
  ...
  modified: 2026-08-29T20:47:12.948Z
---
...
## 累計成長記錄（2026-08-30 新增）

記憶體頁多一欄「累計成長」＋「清除成長記錄」按鈕，存在 `MiliUI_DB.perf.growth`
（`since` / `samples` / `addons[folder]=KB`），**跨登出留著**，只能手動清。

- **只累加正的差值**：每次測量跟上一次比，漲了才加、跌了不扣。回落是 GC 收走的，
  把它扣回去會讓垃圾製造機的指紋變成一條零線——那正是這一欄要抓的東西。
- **這個累加法剛好跨得過登出**：新登入的第一次測量沒有「上一次」可比，不會產生
  差值，所以堆被清空不會被誤記成一筆巨大的成長或負成長。
- **只在自動測量勾著時累加**。手動按「重新測量」不算——使用者要的是「勾選之後
  這段時間」，混進零星的手動測量會讓區間失去意義。
- **有記錄時清單改依成長排序**（沒有就回到 MB 由大到小），表頭「累計成長」染
  強調色表示「現在照這一欄排」，狀態列寫明記錄起始時間與測量次數——記錄跨登入
  留著，不講起始時間的話玩家看到一筆 300MB 不知道那是十分鐘還是三天。
- 欄位空間是跟**長條**借的（148 → 92），不是跟名稱借的：名稱被截斷比長條短一截
  難用得多。
- **⚠ 它量的是「留下來的」，不是「配置過的」。** 每 5 秒取一次快照、比相鄰兩點的
  差，中間被 GC 收走的完全看不見——一個每秒 churn 1 MB 但立刻被回收的插件，淨差
  是 0，這欄永遠是 0。所以它抓得到「留著不放」（快取越長越大、洩漏），抓不到
  垃圾製造機；後者的指紋在「變化」欄（持續 + 然後一大筆 −）。
  **累加端沒有門檻**（任何正差值都加），只有顯示端有 0.5 MB 的地板，所以短時間
  全是「—」是正常的，不是壞掉。這兩件事要寫在 footer 與勾選框的工具提示裡：
  沒講的話「全 0」會被誤讀成「沒有插件在吃記憶體」。
