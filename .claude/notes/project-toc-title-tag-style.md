...
metadata: 
  ...
  modified: 2026-09-07T13:20:56.019Z
---
...
2. 沿順序等距移色相：HSL S=100%、L=60%；介面從 **213°（藍）起每階 −6°**，尾端由
   標籤數決定（12 個到 147°、13 個到 141°）。
3. 同標籤同色（團隊×2、地圖×2 各自共用一色）。
4. 新增插件進該分類時，**整條漸層重算重上**，不是插一個中間色。
   ⚠ 2026-09-07 就是忘了這條：[筆記]（E8C56C）跟 [資訊]（00FFFF）各自挑了色直接塞進去，
   使用者一眼看出漸層斷掉。以後加標籤一律跑下面的腳本重鋪整條。
