...
**第三輪（2026-09-18，commit 540cad5a0）：橫向沒空間的兩處改成「往下長」**——頭像單位分頁左欄右邊就是分隔線、
表單寬 520 已量到極限，不加寬左欄也不加寬視窗（視窗寬牽動分頁列、搜尋框、其他分頁的絕對座標）。
- `W.WrapButton(b, width, minH)`：FitButton 的另一半。判準是「自然寬 ≤ width 就完全不動」，內距只有換行時才留
  （拿內距當判準，貼邊但沒溢出的中韓譯名會當場多一行）。先拆掉上一輪的夾寬再量，所以可重複呼叫。
  **量不到字高就整個收手退回不換行**——「換了行卻沒長高」會讓第二行畫到下一顆身上，比溢出更糟。
  呼叫端要用回傳高度**累加**著排，而且在分頁真的 Show 之後再排一次（未顯示的框可能量到 0）。
- `W.FlowLayout(parent, buttons, maxW, gapX, gapY, rowH)`／`W.FlowRows(buttons, maxW, gapX)`：一排按鈕放不下就換排；
  單排時錨點與「第一顆 TOPLEFT、其餘 LEFT→RIGHT 串接」逐位元相同。maxW 量不到當無限寬（別退成每顆一排）。
  FlowRows 每顆都算（含隱藏的）：**容器高度一次留給最壞情況**，跟著內容跳的話底下的表單每換一次單位就上下彈。
- **卡邊的排要留容忍量**：繁中 chip 全展開 544／548，改成換排之後「估算差 4px」的後果從「伸進留白看不出來」變成
  「突然兩排」⇒ 換排門檻＝容器寬＋8（右邊留白 12 的一部分）。這是驗收時補的，通則：把溢出改成重排時，檢查原本剛好卡邊的語系。
- 量測結果：單位清單中韓 1 行不變，歐語 2–3 行（itIT 最壞整欄 372px，放得進 520）；chip 中文 1 排、韓／英／義西葡 2 排、德俄 3 排。
- InfoBar `Tab_Blocks.lua` 與 Tooltip `Tab_Unit.lua` 各有一份自寫的 `FlowChips`，是 `W.FlowLayout` 可以收掉的重複，現在都正常所以沒動。

**待遊戲內驗證**：歐語客戶端實際看換行後的列高與控件垂直置中、`SetNonSpaceWrap` 對一般歐語斷行有沒有副作用、
按鈕撐寬後的樣子；下拉照最寬項目撐寬後整頁寬度不一的觀感（LSM 材質／字型名很長時中文也會撐）、`IsTruncated()` 提示、勾選框提示換行後的列高、歐語確認彈窗加高；歐語左欄單位鈕有沒有真的換行長高（未顯示時量不到字高的那條風險）、繁中 chip 仍是一排、德俄 chip 三排的觀感。
