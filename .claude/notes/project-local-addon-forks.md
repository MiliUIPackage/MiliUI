...
| **WarpDeplete** | `Core.lua`：藏任務追蹤器改走 alpha（Hide()/Update() 會染污，見 [[project-miliui-hide-blizzard-taint]]）；**2026-09-05 藏追蹤器改成選項** `hideObjectiveTracker`（`DB.lua` 預設 false、`Options.lua` 一般分頁的勾選、`UpdateObjectiveTrackerVisibility()` 切換當場生效、三個語系檔加 key）。M+ 藏清單交給 MiliUI_QuestTracker（[[project-miliui-questtracker]]）。舊存檔沒有這個鍵 ⇒ nil-merge 後即 false，不用遷移 | 上游更新會洗掉三個檔＋語系；重套時注意上游若自己加了同名選項就改用它的 |
