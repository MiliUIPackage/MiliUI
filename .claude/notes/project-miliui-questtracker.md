...
底色 `0x1A`（0.102）＋ alpha `0.8` 跟 `MiliUI_DamageMeters` 的視窗底同一組
（那邊的常數叫 `DARK_BG`，在 `Core/DB.lua`）。**標題列走不透明**，跟傷害統計的
標題列一樣 —— 半透的標題壓在會動的地形上很難讀，而它是主要互動區。
要改配色就兩支一起改，不然兩個視窗擺在一起看得出色差。
滑過提亮走「往白色插值」不是「加常數」，非灰階底色才不會偏色相。

## 右鍵選單

`Modules/Menu.lua` 只有內容，引擎在共用層。**刻意不放**三樣東西，理由寫在檔頭：
...
- 進度條的百分比顯示成 `□%`（缺字方塊）—— 還沒確認是不是我們改字級／描邊造成的。
