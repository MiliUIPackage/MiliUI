...
**PremadeGroupsFilter 的申請者排序錯誤：上游已知、不是套組改壞的（2026-09-21 查證）**。指紋：`LFGList.lua:4197: attempt to compare field 'isNew' (a secret boolean value, while execution tainted by 'PremadeGroupsFilter')`，堆疊整條都是暴雪的（`LFGListFrame_OnEvent → LFGListApplicationViewer_UpdateResultList → LFGListUtil_SortApplicants`），發生在**自己開團招人、秘密值生效中**（團本／M+）有人申請的時候。上游 issue #404（2026-09-09 開、還沒關）一字不差；同類的 #365 從 3 月就有，作者 0xbs 明講：過濾功能本身就得寫 `LFGListFrame.SearchPanel.results`／`totalResults`，「存留報名註記」（`persistSignUpNote`，整支換掉全域 `LFGListApplicationDialog_Show`；`Init.lua` 的預設是關，玩家自己開過才有）和「申請被拒絕的隊伍」（`signUpDeclined`，寫 `LFGListFrame.declines`）污染更廣；污染在進副本之前就發生了，只有 /reload 能清，要根治得整個重做預組清單，所以他選擇先不修。7.6.x 沒修到這條（#404 是在 7.6.2 之後才開的）。本機 PGF 跟上游 7.5.2 的差異只有 TOC 標題、zhTW 字串、`RatingInfo.lua` 的寬度常數，污染面跟上游一樣。**下次看到就直接回答「上游的」，不用再重查**；能給玩家的緩解只有關掉設定裡的「存留報名註記」再 /reload，保證不了歸零。

**PremadeGroupsFilter 第二條：搜尋結果帶秘密表（2026-09-22，已修，未進遊戲驗證）**。指紋：`Modules/Util.lua:48: bad argument #1 to 'pairs' (table expected, got secret)`，堆疊 `LFGListSearchEntry_Update → OnLFGListSearchEntryUpdate → GetSearchResultInfo（Compat.lua:33）→ Table_Copy_Rec`，開一次搜尋幾百筆。它把 `C_LFGList.GetSearchResultInfo` 的結果**整張遞迴複製**，限制生效時其中一個子表是秘密表（`type()` 照樣回 `"table"`、`pairs` 不收）。修在 `MiliUI/Fix/PremadeGroupsFilter_SecretResult.lua`：包掉 `PGF.GetSearchResultInfo`，結果裡有任何讀不了的表就回 nil —— 那是它本來就有的契約，六個呼叫端都有 nil 閘（那一列照暴雪原樣顯示、不過濾）。私有命名空間靠 `PremadeGroupsFilter.Debug` 摸到，上游拿掉就靜默失效。跟上面那條（申請者排序，上游不修）是兩件事。

**PremadeGroupsFilter 第三條：錯誤記在暴雪身上的 `activityIDs`（2026-09-22，已修，未進遊戲驗證）**。指紋：`LFGList.lua:3365: attempt to index field 'activityIDs' (a secret table value, while execution tainted by 'PremadeGroupsFilter')`，堆疊整條暴雪（`LFGListSearchEntry_Update`，兩種入口：`LFG_LIST_SEARCH_RESULT_UPDATED` 找列更新，或 PGF 的 `FilterSearchResults → LFGListSearchPanel_UpdateResults` 當場初始化列）。上游 #406（2026-09-15、未關）同一條，master 也沒改。成因：過濾＝改寫 `SearchPanel.results` 再從插件呼叫 `LFGListSearchPanel_UpdateResults`，`elementData.resultID` 是髒執行建的，暴雪之後一讀就變髒，髒執行不能 index 秘密表；**乾淨的暴雪程式讀秘密表是合法的**，所以清單是暴雪自己建的就不會炸。修在同一支 `PremadeGroupsFilter_SecretResult.lua`：包 `PGF.FilterSearchResults`，`currentSearchResults` 裡有任何讀不了的結果就整次不發佈（第二條已經讓每筆回 nil＝全部保留，發佈只剩弄髒清單這個副作用）。**救不了的尾巴**：限制開始前過濾過的清單本來就是髒的，限制中收到該列更新照樣炸，要等下一次搜尋（限制中重搜一次就回到暴雪自己建的乾淨清單）——插件沒有乾淨的執行可以重建，上游 #403 作者同結論。玩家的緩解就是「開打前沒關預組清單的話，限制中按一次重新整理」。
