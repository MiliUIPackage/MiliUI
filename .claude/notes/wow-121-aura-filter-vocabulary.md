...
**減益黑名單（2026-09-23 開放到玩家／寵物）**：規則 4 的「有時有用」靠
`GetSpellAuraSecrecy` 事先判斷解掉 —— 永遠友方的單位（player／pet）挑選視窗只收
NeverSecret（非 0 的灰掉、手動輸入 ID 也擋），目標類執行期才知敵友，全部放行＋說明。
嗜血疲勞**直接當減益黑名單的預設內容**（`Core/DB.lua` 的 `SATED_DEBUFFS`，首領框除外），
不另做勾選 —— 玩家打開名單就看得到為什麼它不見了。⚠ 因此**移除要記成 false 不是 nil**，
不然 MergeDefaults 下次載入又補回來（刪不掉）；好處是新 ID 加進預設清單，舊設定檔自動補到。
✅ 七個 ID（57723／57724／80354／95809／160455／264689／390435）實測 secrecy 全是 0。
⚠ 還沒實機驗：戰鬥中友方玩家框上疲勞真的消失。
