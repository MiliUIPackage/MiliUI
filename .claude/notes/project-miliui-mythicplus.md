...
- 聊天是比例字型 ⇒ 不能用空白對齊成表格。內文不能自帶色碼，能上色的只有連結與 `{rt4}`（限時，綠三角）／`{rt7}`（超時，紅叉）。
...
- 成績單（1 行）：`{rt4} +14 納羅拉克之穴 限時+1 29:19`。「同隊：四個名字」那行實測 21.5 會折，不放。
...
- 逐人（表頭＋每人一行）：`喔威百買賽 秒傷23.2萬 斷18驅0死2避26%`
  ...
  - 可迴避傷害是選配（**主選單**「可迴避傷害（隊內占比）」開關，只在有效格式是逐人時才列；預設關），寫成**隊內占比**「避26%」省寬度，隊伍要比的本來就是誰吃得多。各自四捨五入，加總可能 99～101%
- 表頭固定 16 寬左右，長副本名（艾拉卡拉，回音之城）已經 19 ⇒ **表頭不能再加東西**（死亡、評分都不放）
...
**待實機驗證**：公會頻道收到 `{rt4}`／`{rt7}` 會不會換成圖示；`|cnIQ4:` 新格式連結從插件送出伺服器收不收；「19 全形」的寬度估算跟實際聊天框對不對得上；打完鑰石到按鈕亮起要等多久。
