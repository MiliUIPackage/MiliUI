...
**格式**（發佈選單最下面一列「格式  逐人 ›」灰字讀數，子選單單選打勾、原地重畫，全頻道共用；**滑過格式選項用提示框預覽這一場實際會送出的文字**）：
...
- 逐人（表頭＋每人一行）：`喔威百買賽 秒傷23.2萬 斷18驅0死2避26%`
  ...
  - 可迴避傷害是選配（子選單「逐人的欄位」打勾，預設關），寫成**隊內占比**「避26%」省寬度，隊伍要比的本來就是誰吃得多
- 表頭固定 16 寬左右，長副本名（艾拉卡拉，回音之城）已經 19 ⇒ **表頭不能再加東西**（死亡、評分都不放）
- 統計有灰色 `?` 標記（總計／殘缺）的場次只給發成績單
