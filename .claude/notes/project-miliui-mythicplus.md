...
**面板欄位**（使用者指定，寬 680）：玩家／分數／戰利品／每秒傷害／承受傷害／可迴避傷害／中斷／驅散／死亡。用語跟傷害統計對齊（zhTW「可迴避傷害」、zhCN「可规避伤害」）。
**每秒值不叫「輸出」**（2026-09-22 使用者指定）。**面板表頭用全稱、聊天發佈用縮寫**（同日第二次指定）：
面板 `L["DPS"]`＝「每秒傷害」、`L["Damage taken"]`＝「承受傷害」（只有表頭用這兩個 key）；
發佈走自己的格式字串（「秒傷%s 斷%d驅%d死%d」、「秒傷第一」），**改表頭時別去動那幾條**。
**表頭右側的「評分 N (+M)」留著**（2026-09-22 使用者想拿掉又改口：加分顯示有價值）。那是完賽當下的 newOverallDungeonScore，
沒加分的場次跟自己那一列的「分數」重複 —— 被問到「是當時還是現在」時答「當時」，別主動提議拿掉。
