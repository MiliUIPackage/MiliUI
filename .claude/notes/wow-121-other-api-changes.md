...
| 遊戲選單 | ✗（改走 snippet） | `MainMenuMicroButtonMixin:OnClick` 第一行是 `IsMouseOver()` 閘，轉發會空轉；全暴雪原始碼沒有第二個 secure 入口，「點擊觸發 ESC 綁定」也沒有 secure 動作可用。`SecureHandlerClickTemplate` 的 `_onclick` 直接 Show／Hide `GameMenuFrame`，並掛一個 secure 空框讓它變隱式保護框（戰鬥中 snippet 才拿得到它）。**插件端 `ShowUIPanel`／`HideUIPanel`／`ToggleFrame` 戰鬥中一律被拒、taint.log 不記**，見 [[project-miliui-infobar]] |
