...
-- （骨架照 MiliUI_BloodlustMusic / MiliUI_Focus 的 Options/Panel.lua）
------------------------------------------------------------
local _, ns = ...
...
local TABS = {
    ...
    { id = "repair",  label = L["TAB_REPAIR"] },
    { id = "readycheck", label = L["TAB_READYCHECK"] },
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUIInfo_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUIInfo_Options")
    ...
    -- 設定開著時在資訊列上蓋「拖曳移動」遮罩（玩家開設定多半就是要調位置），
    -- 關掉就收。跟編輯模式的藍框是兩套視覺。
    panel:SetScript("OnHide", function()
        W.CloseDropdowns()
        ns.SetBarMoveOverlayShown(false)
    end)
    panel:SetScript("OnShow", function()
        ns.SetBarMoveOverlayShown(true)
