	...
	function EncounterDetails:CreateGraphPanel()
		...
		--> build the phase panel
			...
			function g:GetPhaseTexture (index, phase)
				...
				local phaseColor = phase_colors[phase]
				if (not phaseColor) then
					phaseColor = {1, 1, 1}
				end

				texture.texture:SetVertexColor (unpack (phaseColor))
		...
		--> chart frame implementations
			...
			DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "phases_string", "phases:", "GameFontHighlight")
