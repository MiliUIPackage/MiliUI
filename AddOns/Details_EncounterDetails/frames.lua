...
_detalhes.EncounterDetailsTempWindow = function (EncounterDetails)
	...
	local build_options_panel = function()
		...
		local options_frame = EncounterDetails:CreatePluginOptionsFrame ("EncounterDetailsOptionsWindow", Loc ["Encounter Breakdown Options"], 2)
		...
		local on_show_menu = {
			{value = 1, label = Loc ["Inside Raid"], onclick = set, desc = Loc ["Only show the icon while inside a raid."]},
			{value = 2, label = Loc ["In Group"], onclick = set, desc = Loc ["Only show the icon while in group."]},
			{value = 3, label = Loc ["After Encounter"], onclick = set, desc = Loc ["Show the icon after a raid boss encounter."]},
			{value = 4, label = Loc ["Always"], onclick = set, desc = Loc ["Always show the icon."]},
			{value = 5, label = Loc ["Auto"], onclick = set, desc = Loc ["The plugin decides when the icon needs to be shown."]},
		}
...
--		/dump DETAILS_PLUGIN_ENCOUNTER_DETAILS.db.show_icon
	...
	function EncounterDetails:CreateRowTexture (row)
		...
		row.lineText1 = row.textura:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
		row.lineText1:SetPoint ("LEFT", row.textura, "LEFT", 22, -1)
		row.lineText1:SetJustifyH ("LEFT")
		row.lineText1:SetTextColor (1,1,1,1)

		row.lineText4 = row.textura:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	...
	function EncounterDetails:CreateGraphPanel()
		...
		--> build the phase panel
			...
			local damage_label = DetailsFrameWork:CreateLabel (phase_panel, Loc ["Damage Done:"])
			...
			local heal_label = DetailsFrameWork:CreateLabel (phase_panel, Loc ["Healing Done:"])
			...
			phase_panel.report_label = DetailsFrameWork:CreateLabel (phase_panel, Loc ["|cFFffb400Left Click|r: Report Damage |cFFffb400Right Click|r: Report Heal"])
			...
			local phase_on_enter = function (self)
				...
				if (combat) then
					...
					phase_panel.phase_label.text = Loc ["|cFFffb400Phase|r: "] .. self.phase

					local m, s = _math_floor (self.elapsed / 60), _math_floor (self.elapsed % 60)
					phase_panel.time_label.text = Loc ["|cFFffb400Elapsed|r: "] .. m .. Loc ["m "] .. s .. Loc ["s"]
			...
			local phase_on_click = function (self, button)
				if (button == "LeftButton") then
					...
					tinsert (result, Loc ["Details!: Damage for Phase "] .. self.phase .. Loc [" of "] .. (g.combat and g.combat.is_boss and g.combat.is_boss.name or Loc ["Unknown"]) .. ":")
				...
				elseif (button == "RightButton") then
					...
					tinsert (result, Loc ["Details!: Healing for Phase "] .. self.phase .. Loc [" of "] .. (g.combat and g.combat.is_boss and g.combat.is_boss.name or Loc ["Unknown"]) .. ":")
			...
			function g:GetPhaseTexture (index, phase)
				...
				if (not phase_colors [phase]) then
					_detalhes:Msg (Loc ["Phase out of range:"], phase)
		...
		--> chart frame implementations
			...
			DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "phases_string", Loc ["phases:"], "GameFontHighlight")
			EncounterDetails.Frame["phases_string"]:SetPoint ("TOPLEFT", EncounterDetails.Frame, "TOPLEFT", 20, CONST_CHART_TIMELINE_Y_POSITION)
			
			DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "timeamt0", "00:00", "GameFontHighlight")
			...
			for i = 1, 8, 1 do
				...
				DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "dpsamt"..i, "", "GameFontHighlight")
				...
				DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "timeamt"..i, "", "GameFontHighlight")
			...
			for i = 1, EncounterDetails.MaxGraphics do 
				...
				local text = g:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	...
	--window title
		local titleLabel = DetailsFrameWork:NewLabel (titlebar, titlebar, nil, "titulo", Loc ["STRING_WINDOW_TITLE"], "GameFontHighlightLeft", 16, {227/255, 186/255, 4/255})
		titleLabel:SetPoint ("center", BossFrame, "center")
		titleLabel:SetPoint ("top", BossFrame, "top", 0, -7)
	
	--> Nome do Encontro
		DetailsFrameWork:NewLabel (headerFrame, headerFrame, nil, "boss_name", Loc ["Unknown Encounter"], "QuestFont_Large")
		BossFrame.boss_name = headerFrame.boss_name
		BossFrame.boss_name:SetPoint ("TOPLEFT", BossFrame, "TOPLEFT", 100, -46)
		
	--> Nome da Raid
		DetailsFrameWork:NewLabel (headerFrame, headerFrame, nil, "raid_name", Loc ["Unknown Raid"], "GameFontHighlight")
	...
	--summary
	BossFrame.buttonSwitchNormal = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, Loc ["Summary"], "main")
	...
	--spells e auras
	BossFrame.buttonSwitchSpellsAuras = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, Loc ["Timers & Spells"], "spellsauras")
	...
	--phases
	BossFrame.buttonSwitchPhases = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, Loc ["Phases"], "phases")
	...
	--chart
	BossFrame.buttonSwitchGraphic = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, Loc ["Damage Graphic"], "graph")
	...
	--emotes
	BossFrame.buttonSwitchBossEmotes = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, Loc ["Emotes"], "emotes")
...
--> ~emotes
	...
	local refresh_emotes = function (self)
		...
		if (searching) then
			...
			BossFrame.SearchResults:SetText (Loc ["Found "] .. i .. Loc [" matches"])
		...
		if (emote_pool) then
			for bar_index = 1, CONST_EMOTES_MAX_LINES do 
				...
				if (data) then
					...
					bar.lefttext:SetText (min .. Loc ["m"] .. sec .. Loc ["s:"])
	...
	local row_on_mouse_up = function (self)
		...
		local reportFunc = function()
			...
			EncounterDetails:SendReportLines ({Loc ["Details! Encounter Emote at "] .. time, "\"" .. text .. "\""})
		end
	...
	for i = 1, CONST_EMOTES_MAX_LINES do
		...
		line.lefttext = line:CreateFontString (nil, "overlay", "GameFontHighlight")
		...
		line.righttext = line:CreateFontString (nil, "overlay", "GameFontHighlight")
	...
	local emotes_segment_label = DetailsFrameWork:CreateLabel (BossFrame, Loc ["Segment:"], 14, nil, "GameFontHighlight")
	...
	local build_emote_segments = function()
		...
		for index, segment in ipairs (EncounterDetails.charsaved.emotes) do
			tinsert (t, {label = "#" .. index .. " "  .. (segment.boss or Loc ["unknown"]), value = index, icon = segment_icon, texcoord = segment_icon_coord, onclick = on_emote_Segment_select, iconcolor = segment_icon_color})
		end
		return t
	end
	...
	--search box
	local emotes_search_label = DetailsFrameWork:CreateLabel (BossFrame, Loc ["Search:"], 14, nil, "GameFontHighlight")
	emotes_search_label:SetPoint ("topleft", BossFrame, "topleft", 10, -130)
	
	local emotes_search_results_label = DetailsFrameWork:CreateLabel (BossFrame, "", 14, nil, "GameFontNormal", "SearchResults")
	...
	reset.tooltip = Loc ["Reset Search"]
	...
	local report_emote_button = DetailsFrameWork:NewButton (BossFrame, nil, "$parentReportEmoteButton", "ReportEmoteButton", 180, 20, function()
		local reportFunc = function (IsCurrent, IsReverse, AmtLines)
			...
			if (segment) then
				EncounterDetails.report_lines = {Loc ["Details!: Emotes for "] .. segment.boss}
				local added = 0
				...
				EncounterDetails:Msg (Loc ["There is nothing to report."])
			end
		end
	
		local use_slider = true
		EncounterDetails:SendReportWindow (reportFunc, nil, nil, use_slider)
	end, nil, nil, nil, Loc ["Report Results"])
	...
	local emote_report_label = DetailsFrameWork:NewLabel (search.widget, search.widget, nil, "report_click", Loc ["|cFFffb400Left Click|r: Report Line"])
...
--> auras weakauras
	...
	local bossModsTitle = DetailsFrameWork:CreateLabel (BossFrame, Loc ["Boss Mods Time Bars:"], 14, "orange")
	bossModsTitle:SetPoint(10, -85)
	tinsert (BossFrame.EnemySpellsWidgets, bossModsTitle)
	bossModsTitle:Hide()

	local bossSpellsTitle = DetailsFrameWork:CreateLabel (BossFrame, Loc ["Boss Spells and Auras:"], 14, "orange")
...
local PhaseSelectLabel = _detalhes.gump:CreateLabel (PhaseFrame, Loc ["Select Phase:"], 14, "orange")
local DamageLabel = _detalhes.gump:CreateLabel (PhaseFrame, Loc ["Damage Done"])
local HealLabel = _detalhes.gump:CreateLabel (PhaseFrame, Loc ["Healing Done"])
local PhaseTimersLabel = _detalhes.gump:CreateLabel (PhaseFrame, Loc ["Time Spent on Each Phase"])
...
local report_damage = function (IsCurrent, IsReverse, AmtLines)
	...
	tinsert (result, Loc ["Details!: Damage for Phase "] .. PhaseFrame.LastPhaseSelected .. Loc [" of "] .. (PhaseFrame.CurrentSegment and PhaseFrame.CurrentSegment.is_boss and PhaseFrame.CurrentSegment.is_boss.name or Loc ["Unknown"]) .. ":")
...
local Report_DamageButton = _detalhes.gump:CreateButton (PhaseFrame, report_damage, 16, 16, Loc ["report"])
...
local report_healing = function()
	...
	tinsert (result, Loc ["Details!: Healing for Phase "] .. PhaseFrame.LastPhaseSelected .. Loc [" of "] .. (PhaseFrame.CurrentSegment and PhaseFrame.CurrentSegment.is_boss and PhaseFrame.CurrentSegment.is_boss.name or Loc ["Unknown"]) .. ":")
...
local Report_HealingButton = _detalhes.gump:CreateButton (PhaseFrame, report_healing, 16, 16, Loc ["report"])
...
local ScrollCreateLine = function (self, index)
	...
	_detalhes.gump:SetFontSize (name, 14)
	...
	_detalhes.gump:SetFontSize (done, 14)
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> Selecionar o segmento
		...
		local segmentos_string = DetailsFrameWork:NewLabel (frame, nil, nil, "segmentosString", Loc ["Segment:"], "GameFontNormal", 14)
		...
		--> options button
		local options = DetailsFrameWork:NewButton (frame, nil, "$parentOptionsButton", "OptionsButton", 120, 20, EncounterDetails.OpenOptionsPanel, nil, nil, nil, Loc ["Options"])
	...
	--> macro box
		...
		BossFrame.MacroEditBox.BackgroundLabel = DetailsFrameWork:CreateLabel (BossFrame.MacroEditBox, Loc ["macro"])
	...
	-- ~containers ~damagetaken
		...
		DetailsFrameWork:NewLabel (dano_recebido_bg, dano_recebido_bg, nil, "damagetaken_title", Loc ["Damage Taken per Player"], "GameFontHighlight") --localize-me
	...
	--~ability ~damage taken by spell
		...
		DetailsFrameWork:NewLabel (habilidades_inimigas_bg, habilidades_inimigas_bg, nil, "habilidades_title", Loc ["Damage Taken by Spell"], "GameFontHighlight") --localize-me
	...
	-- ~adds �dds
		...
		local adds_total_string = DetailsFrameWork:CreateLabel (container_adds_window, Loc ["damage taken"])
		adds_total_string.textcolor = "gray"
		adds_total_string.textsize = 12
		...
		DetailsFrameWork:NewLabel (container_adds_window, container_adds_window, nil, "titulo", Loc ["Enemy Damage Taken"], "GameFontHighlight")
	...
	-- ~interrupt
		...
		local interrupt_total_string = DetailsFrameWork:CreateLabel (container_interrupt_window, Loc ["interrupts / casts"])
		interrupt_total_string.textcolor = "gray"
		interrupt_total_string.textsize = 12		
	...
	-- ~dispel
		...
		local dispell_total_string = DetailsFrameWork:CreateLabel (container_dispell_window, Loc ["total dispels"])
		dispell_total_string.textcolor = "gray"
		dispell_total_string.textsize = 12	
	...
	-- ~mortes ~deaths ~dead
		...
		local dead_total_string = DetailsFrameWork:CreateLabel (container_dead_window, Loc ["time of death"])
		dead_total_string.textcolor = "gray"
		dead_total_string.textsize = 12	
