...
local DEFAULTS = {
    ...
    -- 但下緣被社交列佔掉一條，所以要比圓形時代再大一階才讀得一樣舒服。
    -- ⚠ 上下限跟拉把手共用（Map/Skin.lua 的 MIN_SIZE / MAX_SIZE = 100 / 400），
    --   設定頁的滑桿範圍也要跟著那兩個數字，不要各寫各的。
    size      = 240,
    scale     = 1,
    -- 位置存 TOPRIGHT 相對 UIParent TOPRIGHT 的位移，所以 x/y 都是負的 ——
    -- 讀法是「距右緣 8、距上緣 8」。
    --
    -- 小地圖是**釘在右上角**的東西，錨右上角才會在改變解析度或改尺寸時留在原地；
    -- 錨 CENTER 的話換螢幕就會跑掉，錨左上角則是一改尺寸就往右長出畫面。
    --
    -- ⚠ **兩個數字必須相同。** 這是「上緣與右緣等距」的唯一寫法，而等距是這個
    --   位置唯一該有的性質 —— 它是螢幕角落的元件，兩邊留白不一樣會被看成沒對齊。
    --   使用者手動拖出來的是 -9 / -8（在抓等距，差了 1px），取 8 收齊。
    x = -8, y = -8,
    locked    = true,
    ...
    btnGap     = 3,
    btnColumns = 5,
    -- 收納袋那顆九宮格圖示用職業色。關掉就跟「公會」「好友」的標籤同色（白），
    -- 整條社交列變成一個顏色都沒有的讀數列。
    btnAccentIcon = true,
    -- 常駐排貼哪一邊。**預設下方**：收納袋按鈕搬進社交列之後，整個元件由上而下
    -- 就是「地圖 → 社交列 → 常駐排」一路往下疊，釘出來的按鈕接在社交列底下才
    -- 是同一條軸線上的下一段。選 top 會讓它跑到地圖**上面**，跟其他東西斷開。
    -- （bottom 會自動避開社交列，接在它下緣，見 Map/Buttons.lua 的 Layout。）
    ...
    ------------------------------------------------------------
    infoBar          = true,
    -- 25 而不是 18：這條高度同時決定**收納袋那格的圖示邊長**（正方形格＝高減兩條
    -- 邊框），18 會讓那顆 3×3 圖示縮到 16px、在一堆文字旁邊小得像個瑕疵。
    -- 25 讓圖示回到 23px，跟按鈕收納袋裡的 24px 一致。
    infoBarHeight    = 25,
    infoBarGap       = 2,        -- 與地圖之間的縫
    infoBarAttached  = true,     -- 貼著地圖走；關掉就自己記位置
    -- 只有取消「貼在地圖下緣」時才用得到。算出來的值要**剛好落在貼齊時的位置**
    -- （地圖上緣 8 ＋ 邊長 240 ＋ 間距 2 = 250），否則玩家一取消貼齊，社交列就
    -- 跳到別的地方，看起來像設定壞掉。
    infoBarX = -8, infoBarY = -250,
    -- 跟地圖上那兩條帶同字級（12）。
    -- ⚠ 這裡原本寫著「要比地圖的 11 大一階才不會被當成附註」—— 地圖那邊 2026-08-29
    --   也調成 12 之後那句就不成立了，別再照那個理由推。社交列的主次是靠**位置**
    --   （獨立一條、有自己的框）與**職業色的數字**分出來的，不是靠字級。
    infoBarFontSize  = 12,
