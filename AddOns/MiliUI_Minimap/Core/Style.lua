...
--
--   小地圖、社交列、傷害統計視窗、單位框都是前者；設定面板、選單、彈窗是後者。
------------------------------------------------------------
local _, ns = ...
...
function S.SetFont(fs, size, outline)
    ...
    --
    -- 兩個都開有兩個問題：
    --   1. 陰影是 (1, -1)，只往右下 ⇒ 整塊字的墨水**下緣比上緣多 1px**。
    --      字在格子裡明明是幾何置中的，看起來卻偏低 —— 使用者回報的
    --      「文字和數字沒有垂直置中的感覺」就是這 1px。
    --      （描邊是四周均勻的，不會造成偏移。）
    --   2. 描邊已經把字跟背景分開了，再疊一層陰影只是把邊緣糊掉。
    --
    -- 所以：有描邊就不要陰影；沒描邊（NONE）才用陰影撐可讀性 ——
    -- 那個情況字是直接壓在地形上的，總得有一樣東西把它跟背景分開。
    ------------------------------------------------------------
    if outline == "" then
