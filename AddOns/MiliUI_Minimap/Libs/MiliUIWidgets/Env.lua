...
--         MiliUIBurst / MiliUIBLM / MiliUIDM / MiliUIAura / MiliUINote
Env.NAMESPACE = "MiliUIMap"

Env.L = ns.L

Env.P = ns.P

-- 在地化字型（Core/Style.lua 在 TOC 排在本檔之前）
function Env.Font(token)
    return ns.Style.Font(token)
end

-- 強調色 = 玩家職業色（跟地圖邊框用的是同一個來源）
function Env.Accent()
    return ns.Style.Accent()
end
...
-- 標籤欄寬：本插件的 zhTW 標籤偏長（「隱藏郵件／製作訂單圖示」）
Env.LABEL_W = 210
