...
-- 編解碼
------------------------------------------------------------
-- 匯出的是**目前這份設定檔**，不是整個帳號。帳號層的東西（設定視窗位置、
-- 其他設定檔）不該跟著字串跑。
...
function Share.Decode(text)
    ...
    if text:sub(1, #WIRE_PREFIX) ~= WIRE_PREFIX then
        return nil, L["Wrong prefix (not a MiliUI Tooltip export string)"]
    end
...
-- 只留預設值裡真的存在的鍵。多出來的 MergeDefaults 清不掉（它只補 nil），
-- 之後設定分頁與預覽會拿著這個假鍵去跑，每次開面板重演一次。
-- unit 底下同理：只留認得的單位種類（player／npc）。
-- ⚠ 只過濾到這兩層，不做深層的「不在預設就刪」——elements 那種「數字鍵存版面、
-- 字串鍵存元素」的開放式表會被削掉一半。
local function Sanitize(p)
    local ref = ns.DB.ProfileDefaults()
    local out = {}
    for k in pairs(ref) do
        if p[k] ~= nil then out[k] = p[k] end
    end
    if type(out.unit) == "table" then
        local kinds = {}
        for k, v in pairs(out.unit) do
            if ref.unit[k] ~= nil and type(v) == "table" then kinds[k] = v end
        end
        out.unit = kinds
    end
    return out
...
-- 分頁 UI
--
-- 這個分頁沒有預覽（Options/Preview.lua 的 SetForTab 會把它收起來），
-- 所以左右欄的分隔線也跟著隱藏、版面用滿整個視窗寬度。
------------------------------------------------------------
local COL_W  = 470        -- 匯出／匯入各一欄
local COL2_X = 506
local FULL_W = 964
...
local function Init()
    ...
    local profDD = W.CreateDropdown(tab, 230, ProfileItems(), function(value)
        ...
        if not switchConfirm then
            switchConfirm = W.CreateConfirmPopup(ns.Options.panel, 320,
                L["Switch profile? The UI reloads so every tooltip picks up the new settings."],
    ...
    local exportBox = W.CreateScrollEditBox(tab, COL_W, 228)
    ...
    importTitle:SetPoint("TOPLEFT", COL2_X, -122)
    ...
    local importBox = W.CreateScrollEditBox(tab, COL_W, 228, function(eb, userChanged)
    ...
    importBox:SetPoint("TOPLEFT", COL2_X, -154)
    ...
    note:SetPoint("TOPLEFT", 12, -420)
    note:SetWidth(FULL_W)
    note:SetJustifyH("LEFT")
    note:SetText(L["The export string contains this profile's settings only. \"Import and reload\" only lights up once a valid string is pasted."])

    ---------------------------------------------------------
    -- 重置
    ---------------------------------------------------------
    local resetTitle = W.CreateSectionTitle(tab, L["Reset"], FULL_W)
    resetTitle:SetPoint("TOPLEFT", 12, -442)

    local resetBtn = W.CreateButton(tab, L["Restore all defaults and reload"], "red", 150, 22)
    resetBtn:SetPoint("TOPLEFT", 12, -474)

    local resetConfirm
    resetBtn:SetScript("OnClick", function()
        if not resetConfirm then
            resetConfirm = W.CreateConfirmPopup(ns.Options.panel, 320,
                L["Restore this profile (style, health bar, units, items, spells, anchor) to its defaults and reload the UI?"],
    ...
    resetNote:SetWidth(780)
    resetNote:SetJustifyH("LEFT")
    resetNote:SetText(L["Only this profile is reset; the other profiles are left alone. The /mtip reset command does the same thing as this button."])
end
