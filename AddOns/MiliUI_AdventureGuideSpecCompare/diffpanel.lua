...
local function CreatePanel()
    ...
    panel = CreateFrame("Frame", "AGSCPanel", UIParent, "BasicFrameTemplate")
    ...
    -- 「對照天賦不顯示共用裝備」勾選框（固定右側）
    local cb = CreateFrame("CheckButton", "AGSCHideShared", panel, "UICheckButtonTemplate")
    cb:SetSize(22, 22)
    ...
    cbLabel:SetText("基準欄隱藏共用")
    cb:SetChecked(ns.db and ns.db.baselineHideShared or false)
    cb:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_LEFT")
        GameTooltip:AddLine("基準欄隱藏共用裝備")
        GameTooltip:AddLine("勾選後，最左邊「對照天賦」欄不顯示\n全系共用的裝備（避免與上方重複）。", 1, 1, 1, true)
        GameTooltip:Show()
    end)
    cb:SetScript("OnLeave", GameTooltip_Hide)
    cb:SetScript("OnClick", function(self)
        if ns.db then ns.db.baselineHideShared = self:GetChecked() and true or false end
        Refresh()
    end)
    panel.hideSharedCheck = cb
...
local function BuildView()
    ...
    local hideShared = ns.db and ns.db.baselineHideShared
    ...
    for itemID, specMap in pairs(ns.itemSpecMap) do
        ...
        if info then
            ...
            -- 基準天賦全部裝備（勾選「不顯示共用」時排除全系共用）
            if baseHas and not (usedByAll and hideShared) then
    ...
    return {
        universal = universal,
        baseline  = baseSpec and { spec = baseSpec, all = baseAll } or nil,
        columns   = columns,
    }
end
...
local function placeColSection(content, list, label, color, x, y)
    local h = acquireSub(COL_INNER)
    h:SetParent(content)
    h:SetPoint("TOPLEFT", content, "TOPLEFT", x + COL_PAD, y)
    h.label:SetText(string.format("|c%s%s (%d)|r", color, label, #list))
    h:Show()
    y = y - SUBHEADER_H
...
Refresh = function()
    ...
    if view.baseline then
        ...
        cy = placeColSection(content, view.baseline.all, "全部", "ffffd200", 0, cy)
