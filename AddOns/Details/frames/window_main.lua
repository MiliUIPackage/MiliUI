...
local resizeTooltip = {
	{text = Loc ["|cff33CC00Click|cffEEEEEE: "].. Loc ["STRING_RESIZE_COMMON"]},
	
	{text = Loc ["+|cff33CC00 Click|cffEEEEEE: "] .. Loc ["STRING_RESIZE_HORIZONTAL"]},
	{icon = [[Interface\AddOns\Details\images\key_shift]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},
	
	{text = Loc ["+|cff33CC00 Click|cffEEEEEE: "] .. Loc ["STRING_RESIZE_VERTICAL"]},
	{icon = [[Interface\AddOns\Details\images\key_alt]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},
	
	{text = Loc ["+|cff33CC00 Click|cffEEEEEE: "] .. Loc ["STRING_RESIZE_ALL"]},
	...
	if (self.mouse_down and (self.mouse_down+0.4 > _GetTime() and (x == self.x and y == self.y)) or (x == self.x and y == self.y)) then
		if (self.button == "LeftButton" or self.button == "MiddleButton") then
					...
					if (custom and custom.on_shift_click) then
						...
						if (func) then
							...
							if (not successful) then
								_detalhes:Msg (Loc ["error occurred custom script shift+click:"], errortext)
							end
							return
						end
					end
			...
			if (not self.minha_tabela) then
				return _detalhes:Msg (Loc ["this bar is waiting update."])
			end
			...
			if (diff and (diff == 15 or diff == 16) and (attribute == 1 or attribute == 2)) then
				...
				if (db) then
					...
					if (bestRank) then
						...
						GameCooltip:AddLine (Loc ["Best Score:"], _detalhes:ToK2 ((bestRank[1] or 0) / encounterTable.elapsed) .. Loc [" [|cFFFFFF00Rank: "] .. (rankPosition or "#") .. "|r]", 1, "white")
						_detalhes:AddTooltipBackgroundStatusbar()
...
local alert_on_click = function (self, button)
	if (self.func) then
		...
		if (not okey) then
			_detalhes:Msg (Loc ["error on alert function:"], errortext)
		end
	end
	self:GetParent():Hide()
end
...
do
	...
	local text = tooltip_anchor:CreateFontString (nil, "overlay", "GameFontHighlight")
...
-- instance mini widgets -------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> overall data warning
		instancia.overall_data_warning = backgrounddisplay:CreateFontString (nil, "overlay", "GameFontHighlight")
	...
	--> freeze icon
		...
		instancia.freeze_texto = backgrounddisplay:CreateFontString (nil, "overlay", "GameFontHighlight")
	...
	if (text) then
		if (type (text) == "table") then
			...
			if (text.size) then
				...
				_detalhes:SetFontSize (statusbar.text, 12)
			end
		else
				...
				if (atributo == 5) then --> custom
					...
					if (modo == 1) then --alone
						...
						if (SoloInfo) then
							...
							CoolTip:AddMenu (2, OnClickNovoMenu, index, instancia, nil, "#".. index .. " "..Loc ["Unknown Plugin"], _, true)
						end
	...
	if (parameters_table[2] > 0.15) then
				...
				if (thisCombat) then
					...
					if (thisCombat.is_mythic_dungeon_segment) then
						...
						if (mythicDungeonInfo) then
							...
							CoolTip:AddLine (Loc ["STRING_SEGMENT_END"] .. ":", thisCombat.data_fim or Loc ["in progress"], 2, "white", "white")
		...
		----------- current
			...
			if (not segment_info_added) then
				...
				CoolTip:AddLine (Loc ["STRING_SEGMENT_END"] .. ":", _detalhes.tabela_vigente.data_fim or Loc ["in progress"], 2, "white", "white") 
			end
		...
		CoolTip:AddIcon ([[Interface\QUESTFRAME\UI-Quest-BulletPoint]], "main", "left", 16, 16, nil, nil, nil, nil, "orange")
			...
			for i, segment in _ipairs (combats_added) do
				...
				CoolTip:AddLine ("" .. name, minutos..Loc ["m "]..segundos..Loc ["s"], 2, "white", "white")
...
----------> call widgets handlers	
	...
	if (not just_updating or _detalhes.initializing) then
		...
		if (callbackFunc) then
			...
			if (not okey) then
				_detalhes:Msg (Loc ["|cFFFF9900error on skin callback function|r:"], result)
			end
		end
		...
		if (this_skin.control_script) then
			...
			if (onStartScript) then
				...
				if (not okey) then
					_detalhes:Msg (Loc ["|cFFFF9900error on skin control on start function|r:"], result)
				end
			end
...
local atributo_on_enter = function (self, motion, forced, from_click)
	...
	elseif (instancia:IsRaidMode()) then
		...
		if (not have_plugins) then
			...
			GameCooltip:SetOption ("TextHeightMod", 0)
			GameCooltip:SetOption ("IgnoreButtonAutoHeight", false)
			GameCooltip:AddLine (Loc ["All raid plugins already\nin use or disabled."], nil, 1, "white", nil, 14, SharedMedia:Fetch ("font", "Friz Quadrata TT"))
