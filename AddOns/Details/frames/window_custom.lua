...
--> create the window
	...
	function _detalhes:OpenCustomDisplayWindow()
		...
		if (not _G.DetailsCustomPanel or not DetailsCustomPanel.Initialized) then
			...
			--> menu title
				local titleLabel = _detalhes.gump:NewLabel (titlebar, titlebar, nil, "titulo", "Details! ".. Loc ["Custom Displays"], "GameFontNormal", 16)
			...
			local import_display = function (_, _, custom_object, index)
				...
				if (not custom_window.ImportBox) then
					...
					local export_string = gump:NewLabel (custom_window, custom_window, "$parenImportLabel", "exportLabel", Loc ["Import String:"], "GameFontNormal") --Loc ["STRING_CUSTOM_PASTE"]
			...
			custom_window:CreateMenuButton (Loc ["STRING_CUSTOM_IMPORT"], "Interface\\ICONS\\INV_MISC_NOTE_02", import_display, nil, nil, nil, Loc ["Import"], {0.00, 0.9, 0.07, 0.93}) --> localize
			...
			--> create the code editbox
				...
				code_editor:Hide()
				code_editor.font_size = 14
				
				local file, size, flags = code_editor.editbox:GetFont()
				code_editor.editbox:SetFont (file, 14, flags)
