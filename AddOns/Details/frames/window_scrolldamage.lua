...
function Details:ScrollDamage()
	if (not DetailsScrollDamage) then
		...
		DetailsScrollDamage:SetTitle (Loc ["Details! Scroll Damage (/details scroll)"])
		...
		local headerTable = {
			...
			{text = Loc ["Spell Name"], width = 100},
			{text = Loc ["Amount"], width = 60},
			
			{text = Loc ["Time"], width = 60},
			--{text = Loc ["Token"], width = 80},
			{text = Loc ["Spell ID"], width = 80},
			--{text = Loc ["School"], width = 80},
		}
		local headerOptions = {
			padding = 2,
		}
		...
		local autoOpenText = DetailsFramework:CreateLabel(statusBar, Loc ["Auto Open on Training Dummy"])
