...
--> details API functions -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function _detalhes:SendReportLines (lines)
		...
		elseif (type (lines) ~= "table") then
			return _detalhes:NewError (Loc ["SendReportLines parameter 1 must be a table or string."])
		end
	...
	function _detalhes:SendReportWindow (func, _current, _inverse, _slider)
		...
		if (type (func) ~= "function") then
			return _detalhes:NewError (Loc ["SendReportWindow parameter 1 must be a function."])
		end
	...
	function _detalhes:SendReportTextWindow (lines)
		...
		if (not _detalhes.copypasteframe) then
			...
			DetailsFramework:CreateTitleBar (_detalhes.copypasteframe, Loc ["Export Text"])
...
--> slider
	...
	local function cria_slider (este_gump)
		...
		_detalhes:SetFontSize (este_gump.linhas_amt, 14)
...
--> whisper taget field
	...
	local function cria_wisper_field (este_gump)
		...
		_detalhes:SetFontSize (este_gump.wisp_who, 14)
...
--> both check buttons
	...
	function cria_check_buttons (este_gump)
		...
		_detalhes:SetFontSize (_G [checkbox:GetName().."Text"], 14)
		...
		_detalhes:SetFontSize (_G [checkbox2:GetName().."Text"], 14)
...
--> frame creation function
	...
	local elvui_skin = function()
		...
		for i = 1, 9 do --window.max_last_buttons
			...
			_detalhes:SetFontSize (b.text, 12)
	...
	local classic_skin = function()
		...
		_detalhes:SetFontSize (window.last_reported_label, 14)
	...
	function _detalhes:ApplyRPSkin (skin_name)
		...
		if (skin) then
			...
			if (not successful) then
				_detalhes:Msg (Loc ["error occurred on report window skin call():"], errortext)
