        ...
        --> skin selection
            ...
            local buildSkinMenu = function()
                ...
                for skin_name, skin_table in pairs (_detalhes.skins) do
                    ...
                    local desc = Loc ["Author"]..": |cFFFFFFFF" .. skin_table.author .. "|r\n"..Loc ["Version"]..": |cFFFFFFFF" .. skin_table.version .. "|r\n"..Loc ["Site"]..": |cFFFFFFFF" .. skin_table.site .. "|r\n\n"..Loc ["Desc"]..": |cFFFFFFFF" .. skin_table.desc .. "|r\n\n"..Loc ["File"]..": |cFFFFFFFF" .. file .. ".tga|r"
                    ...
                    for index, _table in ipairs (_detalhes.savedStyles) do
                        tinsert (loadtable, {value = index, label = _table.name, onclick = function(_, _, index)
                            ...
                            if (compressedData) then
                                _detalhes:ShowImportWindow(compressedData, nil, Loc ["Details! Export Skin"])
                            else
...
do
    ...
    local buildSection = function(sectionFrame)
        local sectionOptions = {
            ...
            {type = "breakline"},
            {type = "label", get = function() return Loc ["Arena Team Color"] end, text_template = subSectionTitleTextTemplate},
			{--team 1 color
                type = "color",
                get = function()
...
do
    ...
    --> update micro displays
        ...
        do --> micro displays
            ...
            DF:NewLabel (sectionFrame, _, "$parentMicroDisplayWarningLabel", "MicroDisplayWarningLabel", Loc ["STRING_OPTIONS_MICRODISPLAYS_WARNING"], "GameFontHighlight", 12, "orange")
        ...
        local sectionOptions = {
            ...
            {--export profile
                ...
                func = function(self)
                    ...
                    if (str) then
                        _detalhes:ShowImportWindow (str, nil, Loc ["Details! Export Profile"])
                    end
                end,
...
do
    ...
    --> auto switch options
        ...
        local sectionOptions = {
            ...
            {type = "label", get = function() return Loc ["Switch by Role Out of Combat"] end, text_template = subSectionTitleTextTemplate},
...
do
    local buildSection = function(sectionFrame)
        ...
        --> streamer plugin - a.k.a. player spell tracker 
			...
			local streamerTitleDesc = DF:NewLabel (sectionFrame, _, "$parentStreamerTitleDescText", "StreamerTitleDescTextLabel", Loc ["Show the spells you are casting, allowing the viewer to follow your decision making and learn your rotation."], "GameFontNormal", 12, "white")
			...
			if (StreamerPlugin) then
				...
				if (tPluginSettings) then
					...
					if (bIsPluginEnabled) then
						...
						local configurePluginButton = DF:NewButton (sectionFrame, _, "$parentConfigureStreamerPluginButton", "configureStreamerPlugin", 100, 20, configure_streamer_plugin, nil, nil, nil, Loc ["Action Tracker Options"], nil, options_button_template)
						...
						local pluginAlreadyEnabled = DF:NewLabel (sectionFrame, _, "$parentStreamerAlreadyEnabledText", "StreamerAlreadyEnabledTextLabel", Loc ["Plugin is enabled. You may disable it on Plugin Management section."], "GameFontNormal", 12, "white")
						...
						local enablePluginButton = DF:NewButton (sectionFrame, _, "$parentEnableStreamerPluginButton", "enableStreamerPluginButton", 100, 20, enable_streamer_plugin, nil, nil, nil, Loc ["Enable Plugin"], nil, options_button_template)
				...
				local pluginDisabled = DF:NewLabel (sectionFrame, _, "$parentStreamerDisabledText", "StreamerDisabledTextLabel", Loc ["Details!: Streamer Plugin is disabled on the AddOns Control Panel."], "GameFontNormal", 12, "red")
		...
		--> event tracker
			...
			local eventTrackerTitleDesc = DF:NewLabel (sectionFrame, _, "$parentEventTrackerTitleDescText", "EventTrackerTitleDescTextLabel", Loc ["Show what's happening near you so the viewer can follow what's going on. Show cooldowns, CC, spell interruption. Useful on any group content."], "GameFontNormal", 12, "white")
			...
			--> enable feature checkbox
				DF:NewLabel (sectionFrame, _, "$parentEnableEventTrackerLabel", "EventTrackerLabel", Loc ["Enable Event Tracker"], "GameFontHighlightLeft")
			...
			--> configure feature button
				...
				local configureEventTrackerButton = DF:NewButton (sectionFrame, _, "$parentConfigureEventTrackerButton", "configureEventTracker", 100, 20, configure_event_tracker, nil, nil, nil, Loc ["Event Tracker Options"], nil, options_button_template)
		...
		--> current dps
            DF:NewLabel (sectionFrame, _, "$parentCurrentDPSAnchor", "currentDPSAnchor", Loc ["The Real Current DPS"], "GameFontNormal")
            sectionFrame.currentDPSAnchor:SetPoint("topleft", sectionFrame, "topleft", startX + 350, startY - 20)

			local currentDPSTitleDesc = DF:NewLabel (sectionFrame, _, "$parentCurrentDPSTitleDescText", "CurrentDPSTitleDescTextLabel", Loc ["Show a frame with DPS done only in the last 5 seconds. Useful for arena matches and mythic dungeons."], "GameFontNormal", 12, "white")
			...
			--> enable feature checkbox
				DF:NewLabel (sectionFrame, _, "$parentEnableCurrentDPSLabel", "CurrentDPSLabel", Loc ["Enable The Real Current Dps"], "GameFontHighlightLeft")
			...
			--> configure feature button
				...
				local configureCurrentDPSButton = DF:NewButton (sectionFrame, _, "$parentConfigureCurrentDPSButton", "configureCurrentDPS", 100, 20, configure_current_dps, nil, nil, nil, Loc ["Current Real DPS Options"], nil, options_button_template)
...
do
    ...
    local titulo_datacharts_desc = DF:NewLabel (sectionFrame, _, "$parentDataChartsText2", "DataCharts2Label", Loc ["STRING_OPTIONS_DATACHARTTITLE_DESC"], "GameFontNormal", 12, "white")
    ...
    local buildSection = function(sectionFrame)
        ...
        local sectionOptions = {
            ...
            {--always in combat
                ...
                name = Loc ["Always in Combat"],
                desc = Loc ["Details won't create new segments for trash or boss and treat the run as a single segment."],
            },
            ...
            {--dedicated segment for bosses
                ...
                name = Loc ["Boss Dedicated Segment"],
            ...
            {--overall only with bosses
                ...
                name = Loc ["Overall Segment Boss Only"],
                desc = Loc ["Only add boss segments on the overall."],
            },
            ...
            {--delete merged trash
                ...
                name = Loc ["Delete Merged Trash Segments"],
                desc = Loc ["After have the segment merged, if this option is enabled it'll delete those merged segments."],
            },            
