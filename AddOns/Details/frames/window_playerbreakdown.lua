...
function gump:TrocaBackgroundInfo()
	...
	elseif (info.atributo == 3) then --> REGEN
		info.bg1_sec_texture:SetTexture (nil)
		info.tipo = 2
		info.targets:SetText (Loc ["Vindo de:"])
...
function gump:CriaDetalheInfo (index)
	...
	info.nome2 = info.bg:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	info.dano = info.bg:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	info.dano_porcento = info.bg:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	info.dano_media = info.bg:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	info.dano_dps = info.bg:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
...
function gump:CriaJanelaInfo()
	...
	--> titulo
	gump:NewLabel (este_gump, este_gump, nil, "title_string", Loc ["STRING_PLAYER_DETAILS"], "GameFontHighlightLeft", 15, {227/255, 186/255, 4/255})
...
--	este_gump.leftbars1_backgound = leftbars1_backgound
	...
	function este_gump:SetStatusbarText (text, fontSize, fontColor)
		if (not text) then
			este_gump:SetStatusbarText (Loc ["Details! Damage Meter | Use '/details stats' for statistics"], 12, "gray")
			return
		end
					...
					if (misc_player.cooldowns_defensive_spells) then
						...
						if (#cooldowns_usados > 0) then
							...
							for i = 1, #cooldowns_usados do
								...
								if (spellInfo) then
									label2:SetText (esta_habilidade[2] .. " (" .. floor (spellInfo.uptime / encounter_time * 100) .. Loc ["% uptime)"])
								else
										...
										for targetName, amountCasted in pairs (targets) do
											if (targetName == player.nome) then
												...
												if (cooldownDuration > 0) then
													label2:SetText (amountCasted .. " (" .. "|cFFFFFF00" .. miscPlayer.nome .. "|r " .. floor (cooldownDuration / encounter_time * 100) .. Loc ["% uptime)"])
												else
...
local function CriaTexturaBarra (instancia, barra)
	...
	barra.lineText1 = barra:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	...
	barra.lineText4 = barra:CreateFontString (nil, "OVERLAY", "GameFontHighlight")
	if (barra.targets) then
