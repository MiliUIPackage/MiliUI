...
function _detalhes:AbreJanelaInfo (jogador, from_att_change, refresh, ShiftKeyDown, ControlKeyDown)
	...
	if (not info.RightSideBar) then
		...
		mergePlayerSpellsLabel:SetText("合併玩家法術")
		...
		mergePetSpellsLabel:SetText("合併寵物法術")
...
function gump:CriaDetalheInfo(index)
	...
	spellInfoBlock.nome2 = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.dano = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.dano_porcento = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.dano_media = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.dano_dps = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")

	spellInfoBlock.middleStringUp = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.middleStringDown = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	spellInfoBlock.middleStringMiddle = spellInfoBlock.bg:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
...
local function cria_textos (este_gump, SWW)
	...
	este_gump.atributo_nome = este_gump:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	...
	este_gump.avatar_attribute = este_gump:CreateFontString(nil, "overlay", "GameFontHighlight")
...
--than do the change
	...
	if (skin) then
		...
		if (not successful) then
			_detalhes:Msg(Loc ["error occurred on skin call():"], errortext)
		...
		_detalhes:Msg(Loc ["skin not found."])
	end
...
_detalhes:InstallPDWSkin ("WoWClassic", {func = default_skin, author = Loc ["Details! Team"], version = "v1.0", desc = Loc ["Default skin."]})
...
_detalhes:InstallPDWSkin ("ElvUI", {func = elvui_skin, author = Loc ["Details! Team"], version = "v1.0", desc = Loc ["Skin compatible with ElvUI addon."]})
...
function gump:CriaJanelaInfo()
	...
	_detalhes.gump:SetFontSize(este_gump.topright_text1, 14)
	...
	_detalhes.gump:SetFontSize(este_gump.topright_text2, 14)
...
--	este_gump.leftbars1_backgound = leftbars1_backgound
	...
	function este_gump:SetStatusbarText (text, fontSize, fontColor)
		if (not text) then
			este_gump:SetStatusbarText (Loc ["Details! Damage Meter | Use '/details stats' for statistics"], 14, "gray")
			return
		end
...
-- ~pet
		...
		local compare_create = function(tab, frame)
			...
			local create_bar = function(name, parent, index, main, is_target)
				local y = ((index-1) * -19) - 7 -- 更改預設值

				local spellicon = parent:CreateTexture(nil, "overlay")
				spellicon:SetSize(18, 18) -- 更改預設值
				...
				local font = SharedMedia:Fetch ("font", "Arial Narrow")
				bar.righttext2:SetFont(font, 15)
...
function _detalhes.playerDetailWindow:monta_relatorio (botao)
	...
	if (not player) then
		_detalhes:Msg (Loc ["Player not found."])
		return
	end
...
local function CriaTexturaBarra(newLine)
	...
	newLine.lineText1 = newLine:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	...
	newLine.lineText4 = newLine:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	if (newLine.targets) then
