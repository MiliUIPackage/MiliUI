			...
			for tindex, thisOption in ipairs(optionsTable) do
				...
				if (show) then
					...
					if (thisOption.font) then
						...
						thisOptionFrame.label:SetFont("GameFontHighlight", 10.5)
					end
...
function DF:CreateNewDropdownFrame (parent, name)
	...
	local text = f:CreateFontString ("$parent_Text", "ARTWORK", "GameFontHighlight")
	text:SetPoint ("left", icon, "right", 5, 0)
	text:SetJustifyH ("left")
	text:SetText ("未選擇任何選項")
...
function DF:CreateDropdownButton (parent, name)
	...
	local text = f:CreateFontString ("$parent_Text", "OVERLAY", "GameFontHighlight")
	text:SetPoint ("left", icon, "right", 5, 0)
	text:SetJustifyH ("left")
	DF:SetFontSize (text, 14)
	f.label = text
