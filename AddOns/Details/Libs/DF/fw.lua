		...
		for index, widget_table in ipairs (menu) do
			if (not widget_table.hidden) then
				...
				elseif (widget_table.type == "select" or widget_table.type == "dropdown") then
					...
					local label = DF:NewLabel (parent, nil, "$parentLabel" .. index, nil, widget_table.name .. (use_two_points and ": " or ""), "GameFontNormal", widget_table.text_template or text_template or 14)
...
DF.font_templates ["OPTIONS_FONT_TEMPLATE"] = {color = "yellow", size = 15, font = DF:GetBestFontForLanguage()}
