		...
		if (not row.hidden) then
			...
			if (type == "text") then
				for i = 1, #self.scrollframe.lines do
					...
					DF:SetFontSize (text, row.textsize or 14)
...
local update_rows = function (self, updated_rows)
	...
	for i = 1, #updated_rows do
		...
		if (not raw) then
			...
			DF:SetFontSize (widget.text, raw.textsize or 14)
...
function DF:IconPick (callback, close_when_select, param1, param2)
	...
	if (not DF.IconPickFrame) then 
		...
		--serach
		DF.IconPickFrame.searchLabel =  DF:NewLabel (DF.IconPickFrame, nil, "$parentSearchBoxLabel", nil, "搜尋:")
		...
		DF.IconPickFrame.searchLabel.fontsize = 14
		...
		DF.IconPickFrame.customIcon = DF:CreateLabel (DF.IconPickFrame, "圖示路徑:", DF:GetTemplate ("font", "ORANGE_FONT_TEMPLATE"))
		DF.IconPickFrame.customIcon:SetPoint ("bottomleft", DF.IconPickFrame, "bottomleft", 12, 16)
		DF.IconPickFrame.customIcon.fontsize = 14
...
function DF:CreateScaleBar(frame, config) --~scale
	local scaleBar, text = DF:CreateSlider(frame, 120, 14, 0.6, 1.6, 0.1, config.scale, true, "ScaleBar", nil, "縮放:", DF:GetTemplate ("slider", "OPTIONS_SLIDER_TEMPLATE"), DF:GetTemplate ("font", "ORANGE_FONT_TEMPLATE"))
...
local feedback_get_fb_line = function (self)
	...
	if (not line) then
		...
		line.desc = line:CreateFontString (nil, "overlay", "GameFontHighlight")
...
function DF:CreateChartPanel (parent, w, h, name)
	...
	--div lines
		for i = 1, 8, 1 do
			...
			local s = f:CreateFontString (nil, "overlay", "GameFontHighlight")
			f ["dpsamt"..i] = s
			s:SetText ("100k")
	...
	--create time labels and the bottom texture to use as a background to these labels
		...
		for i = 1, 17 do 
			local time = f:CreateFontString (nil, "overlay", "GameFontHighlight")
			time:SetText ("00:00")
...
function DF:CreateRightClickToClose(parent, xOffset, yOffset, color, fontSize)
	...
	local label = DF:CreateLabel(parent, "點一下右鍵關閉", fontSize, color)
...
function DF:OpenLoadConditionsPanel (optionsTable, callback, frameOptions)
	...
	if (not DetailsFrameworkLoadConditionsPanel) then
			...
			if IS_WOW_PROJECT_MAINLINE then
				...
				do
					...
					local buildTalentMenu = function()
						...
						GameCooltip2:AddLine ("選擇要刪除的天賦 (由其他專精或角色加入的)", "", 1, "orange", "orange", 9)
		...
		--create the text entry to type the encounter ID
			...
			encounterIDEditbox.tooltip = "輸入多個 ID 時用空白分隔。\n範例: 35 45 95\n\n統御聖所:\n"
		...
		--create the text entry for map ID
			...
			mapIDEditbox.tooltip = "輸入多個 ID 時用空白分隔。\n範例: 35 45 95"
...
DF.PowerFrameFunctions = {
	...
	Initialize = function (self)
		...
		if (self.Settings.ShowPercentText) then
			...
			DF:SetFontSize (self.percentText, 12)
...
function DF:CreateUnitFrame (parent, name, unitFrameSettingsOverride, healthBarSettingsOverride, castBarSettingsOverride, powerBarSettingsOverride)
	...
	--> unit frame layers
		do
			--artwork
			f.unitName = f:CreateFontString (nil, "artwork", "GameFontHighlight")
