			...
			if (miscActor and miscActor.buff_uptime_spells) then
				...
				for i = 1, #mainAuras do
					...
					if (spellObject) then
						...
						gump:SetaDetalheInfoTexto(i, 100, FormatSpellString ("" .. spellID .. " " .. spellName), "啟動: " .. spellApplies, " ", "刷新: " .. spellRefreshes, " ", "覆蓋時間: " .. spellUptime .. "s")
						added = added + 1
					end
				end
...
function atributo_damage:MontaDetalhesDamageDone (spellId, spellLine, instance)
	...
	if (spellTable.e_total) then
		...
		if (level1AverageDamage ~= "0") then
			t4[4] = "等級1 平均: " .. level1AverageDamage .. " (" .. (empowerAmountPerLevel[1] or 0) .. ")"
		end

		if (level2AverageDamage ~= "0") then
			t4[6] = "等級2 平均: " .. level2AverageDamage .. " (" .. (empowerAmountPerLevel[2] or 0) .. ")"
		end

		if (level3AverageDamage ~= "0") then
			t4[11] = "等級3 平均: " .. level3AverageDamage .. " (" .. (empowerAmountPerLevel[3] or 0) .. ")"
		end

		if (level4AverageDamage ~= "0") then
			t4[10] = "等級4 平均: " .. level4AverageDamage .. " (" .. (empowerAmountPerLevel[4] or 0) .. ")"
		end

		if (level5AverageDamage ~= "0") then
			t4[5] = "等級5 平均: " .. level5AverageDamage .. " (" .. (empowerAmountPerLevel[5] or 0) .. ")"
		end
	end
