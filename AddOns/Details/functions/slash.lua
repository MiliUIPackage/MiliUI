...
--		/run TTT:SetTexture ("Interface\\1024.tga")
	...
	elseif (msg == "bcollor") then
		...
		print (Loc ["backdrop"], _detalhes.ResetButton:GetBackdrop())
	...
	elseif (command == "guid") then
		...
		if (not _detalhes.id_frame) then 
			...
			_detalhes.id_frame.texto:SetFontObject (GameFontHighlight)			
	...
	elseif (msg == "ilvl" or msg == "itemlevel" or msg == "ilevel") then
		...
		_detalhes:Msg (Loc ["gear score: "] .. item_level, Loc ["| item amount:"], item_amount, Loc ["| ilvl:"], average)
...
--	https://www.dropbox.com/s/ulyeqa2z0ummlu7/1024.tga?dl=0
	...
	elseif (msg == "merge") then
		...
		if (_detalhes.in_combat) then
			_detalhes:Msg (Loc ["already in combat, closing current segment."])
		...
		_detalhes:Msg (Loc ["done merging, segments: "] .. segmentsAdded .. Loc [", total time: "] .. DetailsFramework:IntegerToTimer (totalTime))
	...
	elseif (msg == "saveskin") then
		local skin = Details.skins["Serenity"].instance_cprops -- 更改預設值
	...
	elseif (msg == "9") then
		...
		print (Loc ["current profile:"], _detalhes:GetCurrentProfileName())
		print (Loc ["always use profile:"], _detalhes.always_use_profile)
		print (Loc ["profile name:"], _detalhes.always_use_profile_name)
	...
	elseif (msg == "share") then
		...
		elseif (DetailsFramework.IsTBCWow()) then
			...
			print (Loc ["STRING_DETAILS1"] .. Loc ["|cFFFFFF00GAME VERSION|r: |cFFFFAA00"] .. _detalhes.game_version)
		else
			--retail
			local v = _detalhes.game_version .. "." .. (_detalhes.build_counter >= _detalhes.alpha_build_counter and _detalhes.build_counter or _detalhes.alpha_build_counter)
			print (Loc ["STRING_DETAILS1"] .. Loc ["|cFFFFFF00DETAILS! VERSION|r: |cFFFFAA00R"] .. (_detalhes.build_counter >= _detalhes.alpha_build_counter and _detalhes.build_counter or _detalhes.alpha_build_counter))
			print (Loc ["STRING_DETAILS1"] .. Loc ["|cFFFFFF00GAME VERSION|r: |cFFFFAA00"] .. _detalhes.game_version)
		end
	end
end
