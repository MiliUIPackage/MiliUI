...
function mythicDungeonCharts.ShowReadyPanel()
	...
	if (not mythicDungeonCharts.ReadyFrame) then
		...
		DetailsFramework:CreateTitleBar (f, Loc ["Details! Damage Graphic for M+"])
...
function mythicDungeonCharts.ShowChart()
	...
	if (not mythicDungeonCharts.Frame) then
		...
		--titlebar
			...
			--> title
			local titleLabel = _detalhes.gump:NewLabel (titlebar, titlebar, nil, "titulo", Loc ["STRING_OPTIONS_PLUGINS"], "GameFontHighlightLeft", 14, {227/255, 186/255, 4/255})
		...
		--titlebar when minimized
			...
			--> title
			local titleLabelMinimized = _detalhes.gump:NewLabel (titlebarMinimized, titlebarMinimized, nil, "titulo", Loc ["Dungeon Run Chart"], "GameFontHighlightLeft", 14, {227/255, 186/255, 4/255})
		...
		local enabledSwitch, enabledLabel = Details.gump:CreateSwitch (f, on_switch_enable, _detalhes.mythic_plus.show_damage_graphic, _, _, _, _, _, _, _, _, _, Loc ["Enabled"], Details.gump:GetTemplate ("switch", "OPTIONS_CHECKBOX_BRIGHT_TEMPLATE"), "GameFontHighlightLeft")
		enabledSwitch:SetAsCheckBox()
		enabledSwitch.tooltip = Loc ["Show this chart at the end of a mythic dungeon run.\n\nIf disabled, you can reactivate it again at the options panel > streamer settings."]
	...
	mythicDungeonCharts.Frame.ChartFrame:AddOverlay (bossTimeTable, {1, 1, 1, 0.05}, Loc ["Show Boss"], "")
