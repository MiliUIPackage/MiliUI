...
ns.locale = L
_G.HandyNotes_Silvermoon = addon
...
local NPClinkSilvermoon = CreateFrame("GameTooltip", "NPClinkSilvermoon", UIParent, "GameTooltipTemplate")
local function GetCreatureNameByID(id)
    if (not id) then return end

    NPClinkSilvermoon:SetOwner(UIParent, "ANCHOR_NONE")
    NPClinkSilvermoon:SetHyperlink(("unit:Creature-0-0-0-0-%d"):format(id))
    local name      = _G["NPClinkSilvermoonTextLeft1"]:GetText()
    local sublabel  = _G["NPClinkSilvermoonTextLeft2"]:GetText()
...
local function GetIconScale(icon, picon)
    ...
    -- anvil, decor and quartermaster npcs are vendors
    if (icon == "anvil" or icon == "decor" or icon == "quartermaster") then return ns.db["icon_scale_vendor"] end
...
local function GetIconAlpha(icon)
    -- anvil, decor and quartermaster npcs are vendors
    if (icon == "anvil" or icon == "decor" or icon == "quartermaster") then return ns.db["icon_alpha_vendor"] end
