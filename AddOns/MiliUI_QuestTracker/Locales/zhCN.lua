...
L["\"Navigating\" is the quest the map arrow points at — only one at a time, and it moves to whatever you accept next."] = "「导航中」是地图箭头指向的那一条，同时只会有一条，接新任务会自动切过去。"
...
L["Commands: |cffffd200/mquest|r opens the options, |cffffd200/mquest fold|r folds or unfolds the list, |cffffd200/mquest trace|r logs the quest automation, |cffffd200/mquest reset|r restores the defaults, |cffffd200/mquest debug|r opens the diagnostic report"] = "命令：|cffffd200/mquest|r 打开设置，|cffffd200/mquest fold|r 折叠或展开列表，|cffffd200/mquest trace|r 追踪任务自动化，|cffffd200/mquest reset|r 还原默认值，|cffffd200/mquest debug|r 打开诊断报告"

-- 診斷用的延遲調整
L["Wait before accepting set to %.2fs."] = "接受任务前的等待改成 %.2f 秒。"
L["Usage: /mquest delay <seconds 0-10>"] = "用法：/mquest delay <秒数 0-10>"


-- 學到的任務等待清單
L["Cleared the learned quest delays."] = "已清除学到的任务等待列表。"
L["Quests that need a wait (%.2fs each): %s"] = "需要先等一下的任务（每条都等 %.2f 秒）：%s"

-- 需要稍等一下的任務清單（設定頁）
L["Quests that need a moment"] = "需要稍等一下的任务"
L["A few quests — weekly ones especially — refuse to be accepted the instant their window opens: the window flashes and nothing happens. Nothing in the game tells us in advance which ones, so quests are accepted immediately by default; when one doesn't go through it is remembered here and gets a short wait next time."] = "少数任务（尤其是周常）在窗口一跳出来的瞬间接不上：窗口闪一下就关，什么也没发生。游戏没有提供任何方法可以事先知道是哪几条，所以默认一律秒接；某一条没接成功时就会被记在这里，下次先等一下再接。"
L["The list is shared by every character on the account, so a quest only has to fail once."] = "这份列表是整个账号共用的，所以同一条任务只会失败一次。"
L["Wait first"] = "先等多久"
L["Seconds, and only for the quests listed below — everything else is still accepted the moment it appears. If a listed quest fails even after waiting, this goes up by 0.1 on its own."] = "单位是秒，而且只套用在下面列表里的任务——其余任务仍然是一跳出来就接。列表里的任务如果等了还是没接到，这个值会自己加 0.1。"
L["Remembered"] = "已记住"
L["Remove"] = "移除"
L["Clear the list"] = "清空列表"
L["Forget every quest in the list? They will be learned again the next time they fail."] = "把列表里的任务全部忘掉？下次它们再失败时会重新学回来。"
L["Nothing yet — every quest is accepted instantly."] = "目前是空的——所有任务都是秒接。"

-- 接失败时的提示
L["Couldn't accept \"%s\" — talk to the quest giver again. It will wait %.2fs before accepting next time."] = "没接到「%s」，请再跟任务发布者对话一次。已记住这条，下次会先等 %.2f 秒再接。"

-- 傳奇鑰石面板（Modules/MythicPlus.lua、Options/Tab_MythicPlus.lua）
L["Mythic+"] = "史诗钥石"
L["Mythic+ timer"] = "史诗钥石计时"
L["While a keystone is running the list folds away and this panel takes its place under the title bar: timer with +2/+3 splits, key level and affixes, deaths, bosses with kill times, and the enemy forces bar. It needs \"During a Mythic+ run\" on the Folding tab to be on."] = "钥石进行中列表会折起来，这块面板接在标题栏底下取代它：计时（含 +2／+3 分段）、钥石等级与词缀、死亡数、首领与击杀时间、敌军条。需要“自动折叠”分页的“史诗钥石中”开着。"
L["Show the panel"] = "显示面板"
L["Enemy forces in unit tooltips"] = "单位提示显示敌军数"
L["Sizes"] = "尺寸"
L["Timer size"] = "计时字号"
L["Key level size"] = "钥石等级字号"
L["Text size"] = "文字字号"
L["Boss list size"] = "首领列表字号"
L["Bar height"] = "条的高度"
...
-- /mquest debug 的诊断报告
L["Diagnostic report"] = "诊断报告"
L["Select all"] = "全选"
L["Select all, copy (Ctrl+C), and paste the whole thing to the author."] = "全选、复制（Ctrl+C），把整段贴给作者。"
L["Diagnostic report is open — copy the whole window and paste it to the author."] = "诊断报告已打开，把窗口里的内容整段复制贴给作者。"
L["Verdict A: the game API reports no scenario (IsInScenario=false, GetScenarioInfo=nil, %d tracker widgets shown); last scenario event %s. If you are not in the event this is expected — run /mquest debug again while the problem is on screen. If you are in the event, the client was never told: try /reload — if that fixes it the problem is Lua-side (tell us), if only a full re-login does it is a client/server sync problem (tell Blizzard)."] = "判定 A：游戏 API 说不在场景里（IsInScenario=false、GetScenarioInfo=nil、追踪器 widget 显示中 %d 个）；上一次收到场景事件是 %s。人不在事件里的话这是正常的 —— 请在症状出现的当下再打一次 /mquest debug。人在事件里的话，就是客户端从头到尾没被通知：先试一次 /reload —— /reload 就好表示问题在 Lua 端（回报给我们），非得重新登录才好表示是客户端与服务器的同步问题（回报给暴雪）。"
L["Verdict B: the scenario module never received an Update — the tracker's dispatch loop is broken. Look at the taint list and errors."] = "判定 B：场景模块从头到尾没收到 Update，追踪器的派发循环坏了。看报告里的 taint 列表与报错。"
L["Verdict B: the API has scenario data but Blizzard's scenario module is not displayed (state=%s, hasContents=%s). Look at the taint list and errors — re-login clearing it fits taint."] = "判定 B：API 有场景数据，但暴雪的场景模块没有显示（state=%s，hasContents=%s）。看报告里的 taint 列表与报错；重新登录就好正是 taint 的指纹。"
L["Verdict B: the game reports %d objective-tracker widget(s) shown, but Blizzard's UIWidget module is empty (state=%s, hasContents=%s). Look at the taint list and errors — re-login clearing it fits taint."] = "判定 B：游戏说追踪器的 widget 有 %d 个显示中，但暴雪的 UIWidget 模块是空的（state=%s，hasContents=%s）。看报告里的 taint 列表与报错；重新登录就好正是 taint 的指纹。"
L["Verdict C: the scenario module rendered but the tracker is hidden by this addon (folded / alpha / parent). This is our bug."] = "判定 C：场景模块有画出来，但追踪器被这个插件藏起来了（折叠／alpha／父层）。这是我们的 bug。"
L["Verdict C: the tracker rendered but sits off-screen. This is our position override."] = "判定 C：追踪器有画出来，但位置在屏幕外。这是我们的位置接管出了问题。"
L["Verdict D: nothing fits — %s tried to lay out but no block fit in the tracker's height (%s), so Blizzard's tracker hid itself. No error is raised on this path. The height is computed from the parent frame — see \"OTF height inputs\" in the report."] = "判定 D：排不进去——%s 试过排版，但追踪器的高度（%s）连一个区块都放不下，所以暴雪的追踪器把自己整个藏起来了。这条路不会报错。高度是从父层框算出来的，请看报告里的“OTF height inputs”那一行。"
L["Verdict: API, module and tracker all agree it is visible. If the screen still looks wrong, take a screenshot."] = "判定：API、模块、追踪器三方都说看得见。画面还是不对的话请截图。"
