...
L["This is the switch that hides the tracker during a Mythic+ run. WarpDeplete has its own \"hide the objective tracker\" option, off by default in this package — keep only one of the two on, or they will fight over the same fade."] = "M+ 期間藏清單就是這一條。WarpDeplete 也有自己的「隱藏任務追蹤器」選項，套組裡預設關；兩邊只能開一邊，否則會搶同一個淡出。"
L["While folded"] = "摺疊期間"
