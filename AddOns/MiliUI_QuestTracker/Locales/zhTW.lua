...
L["\"Navigating\" is the quest the map arrow points at — only one at a time, and it moves to whatever you accept next."] = "「導航中」是地圖箭頭指向的那一條，同時只會有一條，接新任務會自動切過去。"
...
L["Verdict A: the game API reports no scenario (IsInScenario=false, GetScenarioInfo=nil, %d tracker widgets shown); last scenario event %s. If you are not in the event this is expected — run /mquest debug again while the problem is on screen. If you are in the event, the client was never told: try /reload — if that fixes it the problem is Lua-side (tell us), if only a full re-login does it is a client/server sync problem (tell Blizzard)."] = "判定 A：遊戲 API 說不在場景裡（IsInScenario=false、GetScenarioInfo=nil、追蹤器 widget 顯示中 %d 個）；上一次收到場景事件是 %s。人不在事件裡的話這是正常的 —— 請在症狀出現的當下再打一次 /mquest debug。人在事件裡的話，就是用戶端從頭到尾沒被通知：先試一次 /reload —— /reload 就好表示問題在 Lua 端（回報給我們），非得重新登入才好表示是用戶端與伺服器的同步問題（回報給暴雪）。"
...
L["Verdict B: the game reports %d objective-tracker widget(s) shown, but Blizzard's UIWidget module is empty (state=%s, hasContents=%s). Look at the taint list and errors — re-login clearing it fits taint."] = "判定 B：遊戲說追蹤器的 widget 有 %d 個顯示中，但暴雪的 UIWidget 模組是空的（state=%s，hasContents=%s）。看報告裡的 taint 清單與錯誤；重新登入就好正是 taint 的指紋。"
...
L["Verdict D: nothing fits — %s tried to lay out but no block fit in the tracker's height (%s), so Blizzard's tracker hid itself. No error is raised on this path. The height is computed from the parent frame — see \"OTF height inputs\" in the report."] = "判定 D：排不進去——%s 試過排版，但追蹤器的高度（%s）連一個區塊都放不下，所以暴雪的追蹤器把自己整個藏起來了。這條路不會報錯。高度是從父層框算出來的，請看報告裡的「OTF height inputs」那一行。"
