...
-- （骨架照 MiliUI_AuraEnhance / MiliUI_Focus 的 Options/Panel.lua）
------------------------------------------------------------
local _, ns = ...
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUIQuest_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUIQuest_Options")

    -- 標題列：看得見的拖曳把手（⠿ 拖曳移動）＋ 標題文字，整條都能拖著移動視窗。
    -- 右鍵把視窗叫回畫面中央。實作在共用層 Libs/MiliUIWidgets/Widgets.lua
    W.CreateTitleBar(panel, ns.PREFIX_COLOR .. L["MiliUI Quest Tracker"] .. "|r  v" .. ns.VERSION, SavePosition)
    ...
    panel:SetScript("OnHide", function()
        W.CloseDropdowns()
        ns.Fire("OptionsShown", false)
    end)
    panel:SetScript("OnShow", function()
        SetCombatLocked(InCombatLockdown())
        -- 開設定多半就是要調位置：通知 Chrome 蓋上搬家遮罩
        ns.Fire("OptionsShown", true)
    end)
...
-- 「這個改動要重載才看得到」的提示
--
-- 只給**真的救不回來**的設定用。設定改了畫面沒動，玩家的第一個結論是插件壞了，
-- 所以寧可多一個對話框也不要讓它靜靜地沒反應。
------------------------------------------------------------
local reloadPopup
function Options.AskReload(text)
    if not panel then return end
    if not reloadPopup then
        reloadPopup = W.CreateConfirmPopup(panel, 340, text, function() ReloadUI() end)
    else
        -- 訊息在共用層是 popup.text 這個 FontString，不是 popup 自己的方法
        reloadPopup.text:SetText(text)
    end
    reloadPopup:Show()
end
