...
local ChatFontLarge = CreateFont("ChatFontLarge")
ChatFontLarge:SetFont("Fonts\\ARHei.TTF", 16, "")
...
strangerGroupToggleButton.Text:SetFontObject(ChatFontLarge)
strangerGroupToggleButton.Text:SetText("Reject all party invitations (except from guild, community, friends, and whisper).")
...
filterTitle:SetText("Chat filtering function")
...
messageToggleButton.Text:SetFontObject(ChatFontLarge)
messageToggleButton.Text:SetText("Block Chinese Chat & Chat Bubbles (channel, say, yell, whisper)")
...
toggleButton.Text:SetFontObject(ChatFontLarge)
toggleButton.Text:SetText("Display Mini Map Button")
...
disableInInstanceToggleButton:SetPoint("TOPLEFT", 16, -66)
disableInInstanceToggleButton.Text:SetFontObject(ChatFontLarge)
disableInInstanceToggleButton.Text:SetText("Auto-disable chat filtering when players are in dungeons, battlegrounds, and arenas.")
...
declineInviteToggleButton:SetPoint("TOPLEFT", 16, -340)
declineInviteToggleButton.Text:SetFontObject(ChatFontLarge)
declineInviteToggleButton.Text:SetText("Custom: Reject invitations with specified keywords in player names.")
...
wordsScrollFrame:SetPoint("TOPLEFT", 40, -368)
...
wordsBackground:SetPoint("TOPLEFT", 40, -364)
...
addAdIdEditBox:SetPoint("TOPLEFT", 40, -481)
...
filterTitle:SetText("Team Invitation Feature")
...
customFilterToggleButton:SetPoint("TOPLEFT", 16, -88)
customFilterToggleButton.Text:SetFontObject(ChatFontLarge)
customFilterToggleButton.Text:SetText("Custom: Hide messages containing the following keywords.")
...
customKeywordsScrollFrame:SetPoint("TOPLEFT", 40, -115)
...
customKeywordsBackground:SetPoint("TOPLEFT", 40, -112)
...
newKeywordEditBox:SetPoint("TOPLEFT", 40, -229)
...
addKeywordButton:SetText("Add/Delete")
...
local function filter(self, event, msg, sender)
    ...
    if LPaddon.checkMessage then
        if string.find(lowercase_message, "[\227-\237]") then
            isFiltered = true
        end
    end
...
local function ShouldHideChatBubble(text, sender)
    ...
    if LPaddon.checkMessage then
        if string.find(lowercase_text, "[\227-\237]") then
            isFiltered = true
        else
            for word in pairs(LPaddon.filterWords) do
                local lowercase_word = word:lower()
...
local function printDungeonDifficulty()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cffffff00Dungeon Difficulty set to " .. difficultyName .. ".|r")
end
...
local function onEvent(self, event, ...)
    if event == "PARTY_INVITE_REQUEST" then
        ...
        if isInviteAllowed(inviterNameForConfirmation) then
            ...
            local message = hyperlink .. "has invited you to join a group."
    ...
    if event == "GROUP_INVITE_CONFIRMATION" then
        ...
        if guid then
            ...
            if name and not isInviteAllowed(name) then
                ...
                local hyperlink = "|Hplayer:" .. fullName .. "|h[" .. fullName .. "]|h"
                local message = hyperlink .. "has requested to join your group."
                DEFAULT_CHAT_FRAME:AddMessage(message, 1, 1, 0)
                printDungeonDifficulty()
            end
        end
...
local function filterinvite(self, event, msg, ...)
    if string.match(msg, "invited you to a group, but you could not accept because you are already in a group") then       
...
local function filterSystemMessage(_, _, message, ...)
    if message:find("has invited you to join a group") or message:find("has requested to join your group") or message:find("Dungeon Difficulty set to") or message:find("invite you to their group") then
        return true
    end
    return false
end
