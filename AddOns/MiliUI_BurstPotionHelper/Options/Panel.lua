...
local TABS = {
    ...
    { id = "list",    label = L["SECTION_LIST"] },
    { id = "macro",   label = L["SECTION_MACRO"] },
    { id = "about",   label = L["TAB_ABOUT"] },
}

-- 視窗位置存在自己的 SavedVariables 裡。Core.lua 的 DEFAULTS 沒有這一格，
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUIBurst_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUIBurst_Options")

    -- 標題列：看得見的拖曳把手（⠿ 拖曳移動）＋ 標題文字，整條都能拖著移動視窗。
    -- 右鍵把視窗叫回畫面中央。實作在共用層 Libs/MiliUIWidgets/Widgets.lua
    W.CreateTitleBar(panel, ns.PREFIX_COLOR .. L["SETTINGS_TITLE"] .. "|r  v" .. ns.VERSION, SavePosition)
    ...
    aboutText:SetText(table.concat({
        ns.PREFIX_COLOR .. L["SETTINGS_TITLE"] .. "|r v" .. ns.VERSION,
        "",
        L["SETTINGS_DESC"],
        "",
        L["ABOUT_MACRO"]:format("|cff33ff33" .. ns.MACRO_LINE .. "|r"),
        L["ABOUT_COMBAT"],
        "",
        L["ABOUT_SLASH"],
        "",
