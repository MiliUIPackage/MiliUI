...
--> create the plugin object
	...
	local CreatePluginFrames = function()
		...
		--using_details_string1:SetText (Loc ["Receives dynamic updates from other Details! users when they change talents and gear"])
		...
		DetailsRaidCheck.ToolbarButton:SetScript ("OnClick", function (self, button)
			...
			if (button == "LeftButton") then
				--> link no food/flask
				local s, added = Loc ["Details!: No Flask or Food: "], {}
			...
			elseif (button == "RightButton") then
				...
				local s = Loc ["Details!: No Pre-Pot Last Try: "]
			...
			elseif (button == "MiddleButton") then
				--report focus aug
				local s = Loc ["Details!: Not using Rune: "]
...
local build_options_panel = function()
	...
	local options_frame = DetailsRaidCheck:CreatePluginOptionsFrame ("DetailsRaidCheckOptionsWindow", Loc ["Details! Raid Check Options"], 1)
