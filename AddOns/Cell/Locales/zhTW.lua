...
L["CLICK_CASTING_HINTS_TIPS"] = "把滑鼠點擊施法設定的法術排成一列顯示在畫面上，附上按鍵與冷卻；解鎖後可以拖動位置。"
L["CLICK_CASTING_HINTS_JUMP_TIPS"] = "點一下前往「工具 > 滑鼠施法提示」，可以把下面這些設定顯示成畫面上的一列圖示。"
L["My Anchor Point"] = "我的錨點"
L["MY_ANCHOR_POINT_TIPS"] = "提示列的哪一個角固定在 Cell 上。列的寬度會隨這個角色的快捷數量變，把面向框架的那一角釘住，換角色時間距才不會跑掉。"
L["Snap to Cell"] = "磁吸到團隊框架"
...
L["Snapped"] = "[已磁吸]"
L["SNAP_TO_CELL_TIPS"] = "把提示列拖到團隊框架旁邊放開，就會記住相對位置並跟著 Cell 一起移動；邊緣夠近會自動對齊。拖遠一點放開就脫離。"
L["Close"] = "關閉"
...
L["PARTY_TARGETS_PANE_TIPS"] = "目標按鈕完全跟著隊伍框走：順序、職責排序、外觀都一樣。左鍵點一下就選取那個單位，隊員沒有目標時那一顆不會出現。目標按鈕只畫血量——名字、血量、護盾與團隊標記；沒有仇恨、光環、冷卻、能量條——血條照姓名板那套依敵我上色。"
