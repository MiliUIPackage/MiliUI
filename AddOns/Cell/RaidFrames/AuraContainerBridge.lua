...
------------------------------------------------------------
-- 2026-08-11: adopted into Cell from MiliUI (Fix/Cell_AuraContainer.lua). This is the
-- combat-proven AuraContainer renderer for the debuff row, the three cooldown
-- indicators and custom buff-icon indicators. RaidDebuffContainer.lua keeps owning
-- the central raid debuffs, dispels and the health-bar overlay.
local _, Cell = ...
...
local function ReadCellIndicatorLayout(cellButton, anchorTo)
    ...
    for _, name in ipairs(anchorTo) do
        ...
        if point then
            ...
            -- 12.1: real unit buttons no longer carry an icon pool (indicator[1] is nil
            -- there), so the configured size from the layout entry is the primary source;
            -- the pool (preview buttons only) is the fallback.
            local sizeW, sizeH = 20, 20
            local es = entry and entry["size"]
            if type(es) == "table" and type(es[1]) == "number" then
                sizeW, sizeH = es[1], es[2] or es[1]
            else
            ...
            local num = (entry and tonumber(entry["num"]))
                or (type(indicator.numPerLine) == "number" and indicator.numPerLine > 0 and indicator.numPerLine)
                or 3
...
local function Refresh()
    ...
    wipe(diag.skips)
    diag.ranAt = GetTime()
    -- Always-on: Cell's manual icon pools for these displays were removed (12.1 rewrite),
    -- so the containers are THE renderer now -- out of combat too. AurasAreSecret() is no
    -- longer a display gate; forceOn/wasSecret kept for /cab compatibility.
    local secret = true
    local justLeftSecret = false
    wasSecret = true
...
EventUtil.ContinueOnAddOnLoaded("Cell", function()
    ...
    driver:SetScript("OnEvent", function(_, event)
        if event == "PLAYER_LOGIN" then
            ...
            if failure then
                print("|cffff7777[Cell]|r Cell 光環容器停用：" .. failure)
                return
            end
            enabled = true
    ...
    local function Reset()
        ...
        print("|cffff7777[Cell]|r 已還原 Cell 光環圖示，本模組停用至下次 /reload")
    end
    ...
    SlashCmdList["MILIUICELLAURACONTAINER"] = function(msg)
            ...
            IterateCellButtons(function(cellButton)
                ...
                EnumerateCustomIconIndicators(cellButton, function(t, spellIDs)
                    if spellIDs[spellID] and not found then
                        found = true
                        print(("|cffff7777[Cell]|r Cell 自訂指示器「%s」有在追它，已鏡射成 AuraSlot"):format(
                            tostring(t["name"] or t["indicatorName"])))
                    end
                end)
            end)
            if not found then
                print("|cffff7777[Cell]|r 沒有任何 Cell 自訂指示器在追它 —— 先去 Cell 設定裡加，本模組會跟著鏡射")
            end
            return
        end
        ...
        if cmd == "list" then
            ...
            if not config then
                print("|cffff7777[Cell]|r 讀不到 Cell.vars.currentLayoutTable[\"indicators\"]")
                return
            end
            ...
            for _, t in pairs(config) do
                if type(t) == "table" and t["indicatorName"] then
                    ...
                    print(("|cffff7777[Cell]|r %s｜type=%s auraType=%s enabled=%s 鏡射=%s 法術=%s"):format(
                        tostring(t["name"] or t["indicatorName"]),
                        tostring(t["type"]),
                        tostring(t["auraType"]),
                        tostring(t["enabled"]),
            ...
            print(("|cffff7777[Cell]|r 共 %d 個指示器"):format(n))
            return
        end
        ...
        if cmd == "where" then
            ...
            IterateCellButtons(function(cellButton)
                ...
                print(("|cffff7777[Cell]|r %s（unit=%s）螢幕位置 (%.0f, %.0f)"):format(
                ...
                for key, container in pairs(containers) do
                    ...
                    print(("|cffff7777[Cell]|r   %s：%s→%s (%.0f, %.0f) 螢幕 (%.0f, %.0f) 大小 %.0fx%.0f 顯示=%s 格數=%s"):format(
            ...
            if n == 0 then print("|cffff7777[Cell]|r 沒有任何「可見的」按鈕掛上容器") end
            return
        end
        ...
        if cmd == "test" then
            forceOn = not forceOn
            Refresh()
            print("|cffff7777[Cell]|r 強制顯示 " .. (forceOn and "開" or "關")
                .. " —— 開啟時 Cell 圖示會淡出，再打一次 /cab test 切回")
            return
        end

        local function line(k, v) print("|cffff7777[Cell]|r " .. k .. "：" .. tostring(v)) end
