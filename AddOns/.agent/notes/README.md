...
這裡放**規則與現況**（某個 API 怎麼變了、某個插件改到哪），跟 `../workflows/` 的
「照著做的步驟」分開。動任何跟 12.1 相關的程式碼之前先翻這裡。

這些檔案同時是 Claude Code 的 agent memory
...
**同步方式** —— 在 repo 根目錄（Interface/）執行：

```
cp ~/.claude/projects/-Applications-World-of-Warcraft--ptr--Interface/memory/*.md AddOns/.agent/notes/
rm AddOns/.agent/notes/MEMORY.md
```

memory 那邊改過就重跑一次，別手動編輯兩邊 —— 以 memory 為準，這裡是匯出結果。
