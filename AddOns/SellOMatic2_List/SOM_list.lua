...
function SellOMatic_List:Check_SellList()
	...
	for bag = 0,NUM_BAG_SLOTS,1 do
		for slot = 1, GetContainerNumSlots(bag), 1 do
			...
			if texture ~= nil then
				itemName, _, _, itemLevel, _, _, _, itemStackCount, _, _, itemSellPrice = GetItemInfo(itemLink);
				item_tooltip:SetBagItem(bag,slot);
				if quality == 1 and SellOMatic.db.profile.list_allowWhite then
					if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then
						add = 1;
					end;
				elseif quality > 1 and item_tooltip:Find(ITEM_BIND_ON_EQUIP,1,3) and SellOMatic.db.profile.list_allowBoE or item_tooltip:Find(ITEM_SOULBOUND,1,3) and SellOMatic.db.profile.list_allowBoP then
					if quality == 2 and SellOMatic.db.profile.list_allowGreen then
						if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then add = 1; end;
					elseif quality == 3 and SellOMatic.db.profile.list_allowBlue then
						if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then add = 1; end;
					elseif quality == 4 and SellOMatic.db.profile.list_allowEpic then
						if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then add = 1; end;
					end;
				elseif quality > 1 and not item_tooltip:Find(ITEM_BIND_ON_EQUIP,1,3) and not item_tooltip:Find(ITEM_SOULBOUND,1,3) then
					if quality == 2 and SellOMatic.db.profile.list_allowGreen then
						if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then add = 1; end;
					elseif quality == 3 and SellOMatic.db.profile.list_allowBlue then
						if SellOMatic_List:Check_Item(SellOMatic.db.profile.sellList,itemName) == 1 then add = 1; end;
	...
	if SellOMatic.db.profile.saveList[1] ~= nil then
		...
		for i=1, #list, 1 do
			for j=1, #SellOMatic.db.profile.saveList, 1 do
				if SellOMatic.db.profile.saveList[j] == items[i] then
...
function SellOMatic:SaveInventory()
	local bag, slot
	local num = 0
	for bag = 0, NUM_BAG_SLOTS, 1 do
		for slot = 1, GetContainerNumSlots(bag), 1 do
			local texture, _, locked, _, _, lootable, itemLink, isFiltered = GetContainerItemInfo(bag, slot);
			-- Check that item exists, is not locked, is not filtered and isn't a container.
			if texture ~= nil and locked ~= 1 and not lootable and not isFiltered then
				local itemName, _, _, _, _, _, _, _, _, _, itemSellPrice = GetItemInfo(itemLink)
				if itemSellPrice > 0 then
					if SellOMatic_List:Check_Item(SellOMatic.db.profile.saveList,itemName) == 0 then
						table.insert(SellOMatic.db.profile.saveList,itemName)
						num = num + 1
					end
				end
			end
	...
	SellOMatic:Print(num.." "..L["item(s) added to the save list"])
end
