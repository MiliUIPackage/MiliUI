...
-- 點一下就跑一串巨集：清目標、指定敦敦、標上橘圈、對它報點（內容見下面的 MACRO_LINES）。
--
...
--
-- ⚠ 順序是「清目標 → 指定 → 標記」，一下到位。巨集的條件是逐行執行時才評估，
--   `/tar` 之後的 `@target` 就是敦敦，不需要分兩下按。
--
-- ⚠ 第一行的 `/cleartarget` 不能拿掉：敦敦在範圍外時 `/tar 敦敦` 會**靜默失敗、
--   目標不變**，沒先清掉的話後面的 `/tm [@target]` 就把記號標在你原本打的怪身上
--   （實際發生過）。先清掉，指定失敗就是空目標，`/tm` 與 `/ping` 都會被
--   `[@target,exists]` 跳過。代價是按下去會失去目前目標——對這顆專用鈕可以接受。
--
