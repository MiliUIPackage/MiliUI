...
local function create_MenuButton(name, parent)
    ...
    -- CrossMark (instead of tick)
    local fcm = f:CreateFontString(name.."CrossMark", "OVERLAY", "GameFontNormal")
    fcm:SetSize(16, 16)
    fcm:SetPoint("LEFT", f, 0, 0)
    fcm:SetTextColor(1, 0, 0)
    fcm:SetText("X")
    fcm:Hide()
    f.CrossMark = fcm
...
function lib:UIDropDownMenu_AddButton(info, level)
	...
	if not info.notCheckable then 
		...
		local crossMark = _G[listFrameName.."Button"..index.."CrossMark"]
        if crossMark then
            crossMark:SetShown(info.hasCross)
        end
        ...
        if crossMark and info.hasCross then
            if (checked) then
                check:Hide()
            else
                crossMark:Hide()
        ...
        if _G[listFrameName.."Button"..index.."CrossMark"] then
            _G[listFrameName.."Button"..index.."CrossMark"]:Hide()
        end
	end
