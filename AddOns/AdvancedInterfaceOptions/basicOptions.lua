...
function E:Init() -- Runs after our saved variables are loaded and cvars have been loaded
  ...
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_Camera", "鏡頭", "AdvancedInterfaceOptions")
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_Chat", "聊天", "AdvancedInterfaceOptions")
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_Combat", "戰鬥", "AdvancedInterfaceOptions")
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_FloatingCombatText", "浮動戰鬥文字", "AdvancedInterfaceOptions")
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_StatusText", "狀態文字", "AdvancedInterfaceOptions")
  AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_Nameplate", "名條", "AdvancedInterfaceOptions")
  local cVarFrame, cVarCategoryID = AceConfigDialog:AddToBlizOptions("AdvancedInterfaceOptions_cVar", "CVar參數瀏覽", "AdvancedInterfaceOptions")
  ...
  SlashCmdList.AIO = function(msg)
    ...
    if not InCombatLockdown() then
      ...
      DEFAULT_CHAT_FRAME:AddMessage(format("%s: 戰鬥中無法調整介面設定", addonName))
    end
  end
  SLASH_AIO1 = "/aio"
  ...
  OnAccept = function()
    for _, info in ipairs(addon:GetCVars()) do
      ...
      if current ~= default then
        print(format("|cffaaaaff%s|r 的值從 |cffffaaaa%s|r 重設為 |cffaaffaa%s|r", tostring(cvar), tostring(current), tostring(default)))
