...
local function newCheckbox(parent, cvar, getValue, setValue)
	...
	local description = _G[cvarTable['description']] or cvarTable['description'] or '沒有說明'
...
subText:SetText('這些選項可以切換軍臨天下所移除的各種遊戲選項。')
...
actionCamModeDropdown.initialize = function(dropdown)
	...
	local sortModeLoc = { "移動時跟隨擺動", "聚焦當前目標", "關閉", "跟隨視野方向" }
	for i, mode in next, sortMode do
		local info = UIDropDownMenu_CreateInfo()
		info.text = sortModeLoc[i]
		info.value = sortMode[i]
...
SubText_Chat:SetText('這些選項可以調整聊天設定。') -- TODO
...
SubText_FCT:SetText('這些選項可以變更浮動戰鬥文字。')
...
SubText_NP:SetText('這些選項讓你可以變更姓名版的選項。')
...
SlashCmdList.AIO = function(msg)
	...
	if not InCombatLockdown() then
		...
		DEFAULT_CHAT_FRAME:AddMessage(format("%s: 無法在戰鬥中變更介面選項", addonName))
	end
end
SLASH_AIO1 = "/aio"
