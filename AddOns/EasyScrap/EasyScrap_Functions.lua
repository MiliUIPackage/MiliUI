...
function EasyScrap:itemMatchesFilter(i)
    ...
    if self:itemInWardrobeSet(self.scrappableItems[i].itemID, self.scrappableItems[i].bag, self.scrappableItems[i].slot) then
        self.scrappableItems[i].filterReason = '快易銷毀：Filtered because item is used in wardrobe set.'
        return true
    else
        return false
    end
...
function EasyScrap:itemMatchesFilter(i)
    ...
    for k, v in pairs(customFilter.rules) do
        if not EasyScrap.filterTypes[v.filterType].filterFunction(i, k) then
            EasyScrap.scrappableItems[i].filterMessage = '快易銷毀: 已過濾因為 '..EasyScrap.filterTypes[v.filterType].filterMessage
            return false
...
function EasyScrap:getTrueAzeriteItemLevel(itemIndex)
    ...
    if #tierInfo[#tierInfo].azeritePowerIDs == 1 then
        if tierInfo[#tierInfo].azeritePowerIDs[1] == 13 then
            ...
            DEFAULT_CHAT_FRAME:AddMessage('快易銷毀：Discovered unknown azerite power ID, please report the following item to the developer: '..item.itemID)
        end
    else
        --print warning about unknown azerite armor with multiple final traits?
        DEFAULT_CHAT_FRAME:AddMessage('快易銷毀：Discovered an azerite item with new final trait(s), please report the following item to the developer: '..item.itemID)
    end
end
