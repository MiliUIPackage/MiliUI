...
--@non-debug@
local buildTimestamp = "90005.5-release";
--@end-non-debug@
...
do
	...
	local function AddToBlizzOptions()
		...
		ProfileOptionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("NameplateCooldowns.profiles", L["Profiles"], L["NameplateCooldowns"]);
	end
...
do
	...
	function GUICategory_General(index)
		...
		do
			local sortModes = {
				[SORT_MODE_NONE] = L["none"],
				[SORT_MODE_TRINKET_INTERRUPT_OTHER] = L["trinkets, then interrupts, then other spells"],
				[SORT_MODE_INTERRUPT_TRINKET_OTHER] = L["interrupts, then trinkets, then other spells"],
				[SORT_MODE_TRINKET_OTHER] = L["trinkets, then other spells"],
				[SORT_MODE_INTERRUPT_OTHER] = L["interrupts, then other spells"],
			};
