...
-- （骨架照 MiliUI_AuraEnhance / MiliUI_Focus 的 Options/Panel.lua，只有一個分頁）
--
-- 這支插件的設定全部需要 /reload 才生效，所以視窗本身很薄：一頁開關 ＋ 一顆
-- 重新載入 ＋ 一顆列出套用狀態。
------------------------------------------------------------
local _, ns = ...
...
-- 分頁建構器：各分頁檔案自己往這裡註冊，Panel 不認得任何一頁的內容
Options.builders = {}
Options.tabs = {}
...
local function ShowTab(id)
    W.CloseDropdowns()
    for tabId, frame in pairs(Options.tabs) do
        if tabId ~= id then frame:Hide() end
    end
    local frame = Options.tabs[id]
    if not frame then
        local builder = Options.builders[id]
        if not builder then return end
        frame = builder()
        Options.tabs[id] = frame
    end
    frame:Show()
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUISkin_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUISkin_Options")

    W.CreateTitleBar(panel, ns.PREFIX_COLOR .. L["MiliUI Skin"] .. "|r  v" .. ns.VERSION, SavePosition)
