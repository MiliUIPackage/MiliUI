...
-- 已用掉的前綴列在 README.md，"MiliUISkin" 是這次新增的一個。
Env.NAMESPACE = "MiliUISkin"

Env.L = ns.L

Env.P = ns.P

-- 在地化字型（Core/Tokens.lua 在 TOC 排在本檔之前）
function Env.Font(token)
    return ns.Tokens.Font(token)
end

-- 強調色 = 玩家職業色
function Env.Accent()
    return ns.Tokens.Accent()
end
...
-- 標籤欄寬：預設 128 塞不下英文原文（"Achievements (loads on demand)"），
-- 而英文是這裡的 fallback 語言
Env.LABEL_W = 170
