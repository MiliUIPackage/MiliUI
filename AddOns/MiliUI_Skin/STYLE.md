...
配方自己掛的 frame script 只有 `ThirdParty/Auctionator.lua` 的兩支（2026-09-24）：
背包清單容器 `ItemListingFrame` 的 `HookScript("OnSizeChanged")`（每次重建的最後一步是
`SetHeight`，群組框與物品格都是切到銷售頁那一刻才借出來、晚於兩個伴隨輪 —— 分類標題列一直沒上皮就是這個），
以及物品格的 `HookScript("OnShow")`—— 勾的是格子的 script 不是它的 mixin 方法（那張表勾了也追不上，
