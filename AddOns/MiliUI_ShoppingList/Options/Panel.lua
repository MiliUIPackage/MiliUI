------------------------------------------------------------
-- 設定視窗：分頁鈕掛視窗上緣外側兼拖曳把手
-- 分頁解耦：ns.Fire("ShowOptionsTab", id)，各分頁檔案自己註冊、懶初始化
-- （骨架照 MiliUI_CharacterNotes / MiliUI_Focus 的 Options/Panel.lua）
------------------------------------------------------------
local _, ns = ...
...
local PANEL_W, PANEL_H = 620, 400
local FORM_W = 560          -- 捲動內容寬度（扣掉捲軸）
...
local function WindowDB()
    if type(ns.db.windows.options) ~= "table" then
        ns.db.windows.options = { x = 0, y = 0 }
    end
    return ns.db.windows.options
end
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUIShop_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUIShop_Options")

    W.CreateTitleBar(panel, ns.PREFIX_COLOR .. L["MiliUI Shopping List"] .. "|r  v" .. ns.VERSION, SavePosition)
    ...
    panel:SetScript("OnShow", function() SetCombatLocked(InCombatLockdown()) end)
