...
f:SetScript("OnEvent", function(__, event, arg1)
    if (event == "ADDON_LOADED" and arg1 == "EditModeExpanded") then
        ...
        lib:RegisterFrame(MicroButtonAndBagsBarMovable, "微型選單", EditModeExpandedDB.MicroButtonAndBagsBar)
        lib:RegisterFrame(StatusTrackingBarManager, "經驗條", EditModeExpandedDB.StatusTrackingBarManager)
        lib:RegisterFrame(QueueStatusButton, "尋求組隊狀態", EditModeExpandedDB.QueueStatusButton)
        
        lib:RegisterFrame(TotemFrame, "圖騰", EditModeExpandedDB.TotemFrame)
        lib:SetDefaultSize(TotemFrame, 100, 40)

        DurabilityFrame:SetParent(UIParent)
        lib:RegisterFrame(DurabilityFrame, "裝備耐久度", EditModeExpandedDB.DurabilityFrame)
        
        VehicleSeatIndicator:SetParent(UIParent)
        VehicleSeatIndicator:SetPoint("TOPLEFT", DurabilityFrame, "TOPLEFT")
        lib:RegisterFrame(VehicleSeatIndicator, "坐騎座位", EditModeExpandedDB.VehicleSeatIndicator)
    elseif (event == "UNIT_PET") and (not petFrameLoaded) then
        petFrameLoaded = true
        lib:RegisterFrame(PetFrame, "寵物", EditModeExpandedDB.PetFrame)
    end
end)
