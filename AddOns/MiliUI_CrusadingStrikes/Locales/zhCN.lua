...
L["In the Tracked Buffs row"] = "已在「追踪的增益」栏"
L["Not found"] = "未找到"
L["Can't check during combat"] = "战斗中无法检查"
L["Unknown"] = "无法判断"
L["bar found"] = "已找到长条"
L["Open Blizzard's Cooldown Manager (Edit Mode → Cooldown Manager) and put Crusading Strikes into the Tracked Buffs row. That row can be shrunk or moved off screen, but it must not be turned off."] = "打开暴雪的「冷却管理器」（编辑模式 → 冷却管理器），把「征战圣击」加进「追踪的增益」栏。那一栏可以缩小或移到屏幕外，但不能关闭。"
...
L["Only applies when the nameplate design puts its cast bar below the health bar; designs that put it above are left alone."] = "只有在姓名板设计把施法条放在生命条下方时才会让位；施法条在生命条上方的设计不会动。"

L["Stacks"] = "层数"
L["Show the stack count"] = "显示层数"
L["Mirrors the stack text from the Cooldown Manager bar (experimental — it turns itself off if the client refuses the value in combat)."] = "镜像冷却管理器长条上的层数文字（实验性功能，战斗中客户端不接受那个值时会自动关闭）。"

-- 外观标签页
L["Size and position"] = "尺寸与位置"
L["Height"] = "高度"
L["Width"] = "宽度"
L["Match the health bar"] = "跟生命条同宽"
L["Fixed width"] = "固定宽度"
L["Gap below the health bar"] = "与生命条的间距"
...
L["mirroring:"] = "镜像中:"
L["Stack mirroring failed:"] = "层数镜像失败过:"
L["Errors:"] = "错误记录:"
