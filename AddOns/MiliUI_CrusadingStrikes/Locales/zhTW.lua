...
L["In the Tracked Buffs row"] = "已在「追蹤的增益」列"
L["Not found"] = "未找到"
L["Can't check during combat"] = "戰鬥中無法檢查"
L["Unknown"] = "無法判斷"
L["bar found"] = "已找到長條"
L["Open Blizzard's Cooldown Manager (Edit Mode → Cooldown Manager) and put Crusading Strikes into the Tracked Buffs row. That row can be shrunk or moved off screen, but it must not be turned off."] = "打開暴雪的「冷卻管理器」（編輯模式 → 冷卻管理器），把「征戰聖擊」加進「追蹤的增益」列。那一列可以縮小或移到畫面外，但不能關閉。"
...
L["Commands: |cffffd200/csaa|r opens the options, |cffffd200/csaa check|r prints a diagnosis, |cffffd200/csaa reset|r restores the defaults"] = "指令: |cffffd200/csaa|r 開啟設定、|cffffd200/csaa check|r 印出診斷、|cffffd200/csaa reset|r 還原預設值"
L["Author: Mili (MiliUI package)"] = "作者: Mili（米利UI套組）"

-- 暴雪入口頁
L["Use /csaa to open options"] = "輸入 /csaa 開啟設定"
L["Version: %s"] = "版本: %s"
L["Open options"] = "開啟設定"
