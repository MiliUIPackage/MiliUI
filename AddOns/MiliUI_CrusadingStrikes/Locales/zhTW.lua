...
L["Nameplate only. Applies when the nameplate design puts its cast bar below the health bar; designs that put it above are left alone."] = "僅名條模式。只有在名條設計把施法條放在血條下方時才會讓位；施法條在血條上方的設計不會動。"
...
L["Height"] = "高度"
L["Width"] = "寬度"
L["Match the bar it is attached to"] = "跟掛載的那條等寬"
L["Fixed width"] = "固定寬度"
L["Gap from the bar it is attached to"] = "與掛載那條的間距"
...
-- 掛載位置
L["Where to attach"] = "掛載位置"
L["Attach to"] = "掛在"
L["Target nameplate, below the health bar"] = "目標名條的血條下方"
L["Ayije_CDM Holy Power bar, above"] = "Ayije_CDM 聖能條上方"
L["Ayije_CDM Holy Power bar, below"] = "Ayije_CDM 聖能條下方"
L["Width follows whatever it is attached to (Appearance → Width). On the Holy Power bar it also follows that bar's fading and scale."] = "寬度跟著掛載的那條走（外觀 → 寬度）。掛在聖能條時也會跟著它的淡出與縮放。"
L["Holy Power bar:"] = "聖能條:"
L["Found (Ayije_CDM)"] = "已找到（Ayije_CDM）"
L["Not found — Ayije_CDM is not loaded, or its Holy Power bar is off for this spec"] = "找不到 —— Ayije_CDM 未載入，或這個專精沒開聖能條"

-- 自動掛載
L["Automatic"] = "自動"
L["(automatic)"] = "（自動）"
L["Automatic: above the Ayije_CDM Holy Power bar when that addon is loaded, otherwise below the target nameplate's health bar."] = "自動：有載入 Ayije_CDM 就掛在它的聖能條上方，沒有就掛在目標名條的血條下方。"
