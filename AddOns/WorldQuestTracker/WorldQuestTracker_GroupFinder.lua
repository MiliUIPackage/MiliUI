...
ff.BuildMenuFunc = function()
	...
	--
	GameCooltip:AddLine (L["Send Invite Whispers"])
	...
	ff:SetScript ("OnUpdate", function (self, deltaTime)
		...
		if (nextInviteWave < 0) then
			...
			if (numMembersInGroup >= 5) then
				...
				WorldQuestTracker:Msg (L["Group is full."])
			...
			if (playerName) then
				...
				if (not ff.PlayersInvited [playerName]) then
					...
					if (inviteCooldown+20 > GetTime()) then
						...
						WorldQuestTracker:Msg (L["Inviting "] .. playerName)
						...
						if (WorldQuestTracker.db.profile.groupfinder.send_whispers) then
							SendChatMessage (L["[World Quest Tracker]: Invite for World Quest '"] .. (ff.QuestName2Text:GetText() or "-") .. "'", "WHISPER", nil, playerName)
						end
...
local ignore_current_quest = function()
	DF:ShowPromptPanel (L["Don't Show Popups for the Quest: "] .. (ff.QuestName2Text:GetText() or "-") .. "?", function() 
		if (ff.CurrentWorldQuest) then
			...
			WorldQuestTracker:Msg (L["Quest "] .. (ff.QuestName2Text:GetText() or "-") .. L[" added to ignore list."])
		end
		ff:HideFrame (true)
	end, function() end)
end
...
ChatFrame_AddMessageEventFilter ("CHAT_MSG_WHISPER", function (_, _, msg)
	if (not WorldQuestTracker.db.profile.groupfinder.send_whispers) then
		if (msg:find (L["World Quest Tracker"])) then
			if (msg:find (L["Invite for World Quest"])) then
				return true
			end
		end
	end
