...
--
-- 骨架照 MiliUI_QuestTracker 的 Options/Panel.lua，但分頁只有兩頁，所以沒有
-- 帶 callback 註冊表：各分頁自己呼叫 `Options.RegisterTab(id, show)` 掛上來，
-- 切換時就地叫一遍。
------------------------------------------------------------
local _, ns = ...
...
-- 捲動內容 ＋ Controls.Build 串接。回傳 content, refreshers, rows
--
-- rows 是 Controls.Build 記下的「每一列落在哪」（`{ spec, top, bottom }`）。
-- 要在某幾列上面疊東西（例如總開關關掉時蓋住底下那組）就靠它定位 ——
-- 版面是表單引擎一列一列堆出來的，只有它知道座標。
...
local function CreatePanel()
    ...
    panel = W.CreateFrame("MiliUIMerchant_Options", UIParent, PANEL_W, PANEL_H)
    ...
    tinsert(UISpecialFrames, "MiliUIMerchant_Options")

    -- 標題列：看得見的拖曳把手（⠿ 拖曳移動）＋ 標題文字，整條都能拖著移動視窗。
    -- 右鍵把視窗叫回畫面中央。實作在共用層 Libs/MiliUIWidgets/Widgets.lua
    W.CreateTitleBar(panel, ns.PREFIX_COLOR .. L["MiliUI Merchant"] .. "|r  v" .. ns.VERSION, SavePosition)
